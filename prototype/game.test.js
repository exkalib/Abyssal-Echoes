...
class FakeElement {
  ...
  replaceChild(child,current){ const index=this.children.indexOf(current);if(index<0)return current;child.parentNode=this;current.parentNode=null;this.children[index]=child;return current; }
  addEventListener(){}
  removeEventListener(){}
  setPointerCapture(){}
...
const sandbox = {
  console, document, confirm:()=>true, addEventListener(){}, requestAnimationFrame:fn=>fn(), setTimeout:fn=>fn(), clearTimeout(){},
...
source += `\n;Object.assign(this.api,{renderSkillDetail,renderSkillBrowser,atkRange,skillRange,skillUseStatus,baseSpd,combatTempoText,combatItem,beginCombatTimeline});`;
...
require('./combat-tempo.test.cjs')({a,reset,sandbox});
...

{
  ...
  sandbox.AbyssApp={versionInfo:()=> '安卓 0.7.5 · 外壳 11 · 资源 1788441440'};
...
{
  const genericKinds=new Set(['smelt','craft','recycle','mess','shipyard']);a.CAMP_BUILDINGS.filter(b=>genericKinds.has(b.kind)).forEach(b=>{const s=reset();Object.keys(a.TECHS).forEach(id=>s.meta.techs[id]=1);Object.keys(a.ITEMS).forEach(id=>s.inv[id]=999);s.meta.built[b.id]=true;s.meta.buildLevels[b.id]=Math.max(1,(b.upgrades||[]).length+1);const box=new FakeElement(),nodes=[];a.renderBuilding(box,b.id);(function walk(node){nodes.push(node);(node.children||[]).forEach(walk);})(box);const top=nodes.find(n=>hasClass(n,'recipe-station-top')),topChildren=top&&top.children.slice();nodes.filter(n=>hasClass(n,'station-product-grid')).forEach(grid=>{const tiles=grid.children.filter(n=>hasClass(n,'station-product'));if(tiles.length<2)return;const original=tiles.find(n=>n.attributes['aria-pressed']==='true')||tiles[0],other=tiles.find(n=>n!==original&&n.attributes['aria-pressed']==='false')||tiles.find(n=>n!==original),originalName=original.innerHTML.match(/<b>([^<]+)<\/b>/)[1],otherName=other.innerHTML.match(/<b>([^<]+)<\/b>/)[1];other.click();let detail=[];(function walk(node){if(node.innerHTML)detail.push(node.innerHTML);(node.children||[]).forEach(walk);})(nodes.find(n=>hasClass(n,'recipe-station-bottom')));assert.match(detail.join(' '),new RegExp(otherName),`${b.id} 必须立即显示第二个配方详情`);original.click();detail=[];(function walk(node){if(node.innerHTML)detail.push(node.innerHTML);(node.children||[]).forEach(walk);})(nodes.find(n=>hasClass(n,'recipe-station-bottom')));assert.match(detail.join(' '),new RegExp(originalName),`${b.id} 必须在不关闭建筑的情况下继续切回原配方`);});if(top)assert.deepEqual(top.children,topChildren,`${b.id} 连续切换配方不得重建上方选择区`);});
}
{
...
}
{
  ...
  assert.equal(droneNodes.filter(n=>hasClass(n,'drone-product')).length,1,'一级无人机坞只能选择一级采集无人机');
}

{
  const s=reset();s.meta.built.droneBay=true;s.meta.buildLevels.droneBay=2;Object.assign(s.inv,{scrap:40,ecomp:40,steel:40,wafer:40});
  assert.equal(a.buildDrone('scout',3),true);assert.equal(s.droneFleet.scout,3,'同型号无人机必须允许批量建造');const before={scrap:s.inv.scrap,wood:s.inv.wood||0};a.advanceTime(6);assert.equal(s.inv.scrap,before.scrap+3);assert.equal(s.inv.wood,before.wood+3,'机群必须按数量随时间自动采集并入库');
  assert.equal(a.buildDrone('phase',1),false,'高级无人机必须由对应无人机坞等级解锁');const checkpointTime=s.droneLastAt;a.updateCheckpoint();a.advanceTime(12);a.restoreCheckpoint();assert.equal(s.droneLastAt,checkpointTime,'无人机结算时间和机群必须进入检查点存档');assert.equal(s.droneFleet.scout,3);
  const box=new FakeElement(),nodes=[];a.renderBuilding(box,'droneBay');(function walk(node){nodes.push(node);(node.children||[]).forEach(walk);})(box);const droneProducts=nodes.filter(n=>hasClass(n,'drone-product'));assert.equal(droneProducts.length,2,'二级无人机坞必须在上半屏显示两种已解锁无人机');assert.ok(nodes.some(n=>hasClass(n,'drone-detail')),'下半屏必须显示所选无人机属性和建造操作');
  const originalDrone=droneProducts.find(n=>n.attributes['aria-pressed']==='true'),originalDroneName=originalDrone.innerHTML.match(/<b>([^<]+)<\/b>/)[1],otherDrone=droneProducts.find(n=>n.attributes['aria-pressed']==='false'),otherDroneName=otherDrone.innerHTML.match(/<b>([^<]+)<\/b>/)[1],droneShellBefore=otherDrone.parentNode.parentNode.parentNode,droneDetailBefore=nodes.find(n=>hasClass(n,'station-detail-workbench'));otherDrone.click();const switchedDrone=[];(function walk(node){if(node.innerHTML)switchedDrone.push(node.innerHTML);(node.children||[]).forEach(walk);})(box);assert.equal(otherDrone.parentNode.parentNode.parentNode,droneShellBefore,'切换无人机时上方选择器必须保持挂载');assert.equal(droneDetailBefore.parentNode,null,'切换无人机时必须只替换下方详情');assert.match(switchedDrone.join(' '),new RegExp('SELECTED DRONE[\\s\\S]*'+otherDroneName+'[\\s\\S]*AUTO COLLECTION'),'无人机配方必须在当前面板中立即切换详情，不能关闭重开');originalDrone.click();const droneBack=[];(function walk(node){if(node.innerHTML)droneBack.push(node.innerHTML);(node.children||[]).forEach(walk);})(box);assert.match(droneBack.join(' '),new RegExp('SELECTED DRONE[\\s\\S]*'+originalDroneName),'无人机坞在同一次打开中必须允许连续切换');
}

{
  ...
  const watch=new FakeElement(),nodes=[];a.renderBuilding(watch,'watch');(function walk(node){nodes.push(node);(node.children||[]).forEach(walk);})(watch);const products=nodes.filter(n=>hasClass(n,'defense-product'));
  assert.ok(nodes.some(n=>hasClass(n,'recipe-station-screen')),'哨戒塔必须改为上方选图标、下方升级的统一分屏工作台');assert.equal(nodes.some(n=>n.className==='operation-row'),false,'哨戒塔不得再使用旧的纵向操作行');
  assert.equal(products.filter(n=>n.innerHTML.includes('简易围墙')).length,1,'已建工事在图标选择区只能出现一次');assert.ok(products.some(n=>/绊索地雷/.test(n.innerHTML)&&/待建造/.test(n.innerHTML)),'已解锁但未建的工事必须作为独立图标可选');
  const profile=nodes.find(n=>hasClass(n,'defense-stat-profile')),confirm=nodes.find(n=>hasClass(n,'station-confirm'));assert.match(profile.innerHTML,/下一级属性[\s\S]*防御 16/,'选中已建工事后下方必须预览下一级属性');assert.equal(confirm.innerHTML,'升级至 Lv 2');
  assert.match(source,/if\(b\.kind==='defense'\)box\.classList\.add\('watch-defense-page'\)/,'哨戒塔必须有独立的防线控制台外观标识');assert.match(products.map(n=>n.innerHTML).join(' '),/icon-defense-wall[\s\S]*icon-defense-trap/,'每种防御工事必须使用自己的专属线性图标');
  const originalDefense=products.find(n=>n.attributes['aria-pressed']==='true'),originalDefenseName=originalDefense.innerHTML.match(/<b>([^<]+)<\/b>/)[1],otherDefense=products.find(n=>n.attributes['aria-pressed']==='false'),otherDefenseName=otherDefense.innerHTML.match(/<b>([^<]+)<\/b>/)[1],defenseShellBefore=otherDefense.parentNode.parentNode.parentNode;otherDefense.click();const switchedDefense=[];(function walk(node){if(node.innerHTML)switchedDefense.push(node.innerHTML);(node.children||[]).forEach(walk);})(watch);assert.equal(otherDefense.parentNode.parentNode.parentNode,defenseShellBefore,'切换工事时上方选择器必须保持挂载');assert.match(switchedDefense.join(' '),new RegExp('(CONSTRUCTION READY|ACTIVE DEFENSE)[\\s\\S]*'+otherDefenseName),'哨戒塔必须在当前面板中立即切换工事详情，不能关闭重开');originalDefense.click();const defenseBack=[];(function walk(node){if(node.innerHTML)defenseBack.push(node.innerHTML);(node.children||[]).forEach(walk);})(watch);assert.match(defenseBack.join(' '),new RegExp('(CONSTRUCTION READY|ACTIVE DEFENSE)[\\s\\S]*'+originalDefenseName),'哨戒塔在同一次打开中必须允许连续切换工事');
}
...
{
  const indexHtml=fs.readFileSync(path.join(__dirname,'index.html'),'utf8'),defenseSymbols=['wall','trap','gun','reinforced','laser','plasma','shield','arc','drone','dome'];defenseSymbols.forEach(id=>assert.match(indexHtml,new RegExp('symbol id="icon-defense-'+id+'"'),id+' 防御工事缺少专属 SVG 图标'));
  const uiCss=fs.readFileSync(path.join(__dirname,'ui-system.css'),'utf8');assert.match(uiCss,/#panel\.watch-defense-page \.recipe-station-top \{[\s\S]{0,180}padding:0 12px[\s\S]{0,180}background:var\(--ui-bg-0\)/,'哨戒塔上半区必须用不透明底色遮住滚动内容');assert.match(uiCss,/#panel\.watch-defense-page \.recipe-station-top \.facility-module-head \{[\s\S]{0,220}top:0[\s\S]{0,220}margin:0 -12px/,'顶部设施栏必须无缝覆盖整个滚动区宽度');
}

{
  const uiCss=fs.readFileSync(path.join(__dirname,'ui-system.css'),'utf8');assert.match(uiCss,/#panel\.recipe-station-page \.recipe-station-top \.facility-module-head \{[\s\S]{0,260}top:0[\s\S]{0,260}margin:0 -12px[\s\S]{0,260}background:var\(--ui-surface-2\)/,'所有设施分屏页的吸顶栏必须全宽不透明，不得让内容从缝隙滑过');assert.match(uiCss,/#panel\.skill-console-page \.skill-nav \{[\s\S]{0,220}top:0[\s\S]{0,220}background:var\(--ui-surface-2\)/,'技能页不得保留负数吸顶偏移');assert.match(uiCss,/#panel\.tasks-console \.task-board-nav \{[\s\S]{0,180}top:0[\s\S]{0,180}box-shadow:/,'任务页吸顶栏必须遮住上方安全区');
}
...
{
  const s=reset();s.meta.built.recycler=true;s.meta.buildLevels.recycler=2;s.player.equip.weapon='knife';s.inv.knife=3;
  const yieldMap=a.equipmentRecycleYield('knife');assert.ok(yieldMap.ingot>0,'锻造级装备必须返还对应世代材料');
  assert.equal(a.recycleEquipment('knife',2),true);assert.equal(s.inv.knife,1,'装备拆解必须支持批量且只扣背包中的闲置数量');assert.equal(s.player.equip.weapon,'knife','正在穿戴的装备不得被回收中心拆除');assert.equal(s.inv.ingot,yieldMap.ingot*2);
  assert.equal(a.recycleEquipment('knife',2),false,'闲置装备不足时不得拆到正在穿戴的装备');
  s.inv.plasmaRifle=1;const fieldYield=a.equipmentRecycleYield('plasmaRifle');assert.ok(fieldYield.titanium&&fieldYield.superconductor,'高阶装备必须返还高阶材料而不是只有废铁');
}
...

{
  ...
  assert.match(css,/#panel\.settings-home \.settings-media\{[^}]*grid-template-rows:repeat\(4,minmax\(40px,auto\)\)/,'音效、音乐、触觉反馈与角色外观必须各自只占一条紧凑设置行');
  ...
  assert.match(source,/function upgradeMastery\(k,refresh\)[\s\S]{0,500}markInlineChange\('mastery',k\)[\s\S]{0,150}toast:false/,'精通升级必须用原卡片高亮反馈，不能弹出遮挡按钮的通知');
  ...
  assert.ok(Number(androidBuild.match(/SHELL_VERSION", "(\d+)"/)?.[1])>=12,'文件迁移能力必须提升到外壳协议 12');
  ...
  const updatePublisher=fs.readFileSync(path.join(__dirname,'..','deploy','publish_android_update.sh'),'utf8');assert.match(updatePublisher,/sqlite3 \/var\/lib\/abyss-echo\/saves\.sqlite3[\s\S]*saves-before-\$build\.sqlite3/,'每次发布前必须在线备份现有 SQLite 云存档');assert.match(updatePublisher,/remote_apk_sha256[\s\S]*if \[\[ "\$remote_apk_sha256" != "\$apk_sha256" \]\][\s\S]*scp "\$apk_source"[^\n]*Abyssal-Echoes\.apk\.new[\s\S]*mv[^\n]*Abyssal-Echoes\.apk\.new[^\n]*Abyssal-Echoes\.apk/,'只有服务器 APK 与本地校验不一致时才允许原子同步，资源热更新不得重复上传大包');assert.match(updatePublisher,/if \[\[ "\$bundle_mode" == "full" \]\][\s\S]*scp -r "\$root_dir\/prototype\/assets"[\s\S]*else[\s\S]*research\.png/,'精简更新不得反复同步整套美术资源');assert.match(updatePublisher,/apkSha256[\s\S]*apkSize/,'签名更新清单必须包含 APK 哈希和大小');assert.match(updatePublisher,/bundle_mode[\s\S]*lean/,'旧外壳迁移时必须能发布小于其限制的精简桥接资源包');assert.match(updatePublisher,/min_shell="\$\{3:-1\}"/,'普通资源发布默认必须兼容全部已发行外壳，原生升级只能显式触发');
  ...
  assert.match(bagSource,/if\(typeof state\.bagPortraitHidden!=='boolean'\)state\.bagPortraitHidden=true/,'没有显式偏好时优先物品，但不能覆盖用户选择展开');
  assert.match(bagSource,/const openMasteryStudy=\(\)=>\{closeDetail\(\);state\.tab='char';state\.charView='skills';state\.skillView='mastery'/,'精通手册复用技能页，选择能力前不得消耗');
  assert.doesNotMatch(bagSource,/openSiteSheet\('masteryBook'/,'背包不得再打开不随局部刷新更新数量的旧研读弹层');
  assert.match(bagSource,/card\.onclick=\(\)=>\{selectedId=id;syncStoredSelection\(\);\}/,'存储物品格只选择，不能直接消耗或强制弹层');
  assert.match(bagSource,/button\.onclick=\(\)=>\{state\.bagItemSelected=id;state\.bagSel=item.slot;syncSelection\(\);save\(\);\}/,'装备物品格只更新选择，保留列表且不穿戴');
  assert.match(bagSource,/selectionDock\(showEquipment,[\s\S]*unequip\(item.slot,refreshEquipment\):equip\(item.slot,id,refreshEquipment\)/,'固定底栏提供独立穿卸确认');
  ...
  assert.match(bagSource,/action\.onclick=\(\)=>\{if\(useBlocked\(id\)\)return;[\s\S]{0,160}useItem\(id,/,'详情内消费仍须经过可用性判断和明确确认');
  assert.match(bagSource,/if\(useBlocked\(selectedId\)\)return;[\s\S]{0,160}useItem\(selectedId,/,'快捷消费不能绕过满血、库存与场景检查');
  assert.match(bagEquipmentDetailSource,/action\.onclick=\(\)=>P\(\)\.equip\[slot\]===selectedId\?unequip\(slot,onMutate\):equip\(slot,selectedId,onMutate\)/,'可选完整详情保留独立的穿戴和卸下确认');
  ...
  assert.match(source,/const BUILDING_ART_FILE=\{research:'research\.png',garden:'garden-v2\.png'\}[\s\S]{0,220}function buildingUiIcon\(id\)/,'建筑图必须统一从版本化实物资源加载，菌圃必须使用专门适配小尺寸的透明新图标');
  assert.ok(fs.existsSync(path.join(__dirname,'assets/building-art-v1/garden-v2.png')),'菌圃新图标资源必须真实存在');
  const enemyIds=vm.runInContext('Object.keys(ENEMIES)',sandbox);
  assert.equal(enemyIds.length,25,'当前25个正式敌人必须全部进入战斗立绘系统');
  enemyIds.forEach(id=>{const portraitId=vm.runInContext(`enemyPortraitId('${id}')`,sandbox),asset=path.join(__dirname,'assets','enemy-portraits-v1',portraitId+'.png');assert.ok(fs.existsSync(asset),id+' 缺少全身怪物立绘');assert.ok(fs.statSync(asset).size>100000,id+' 的怪物立绘文件异常');});
  ...
  assert.equal((html.match(/class="ti"/g)||[]).length,5,'底部必须保留角色、职业、技能、背包、任务五个统一入口');
  for(const tab of ['char','career','skill','bag','task'])assert.match(html,new RegExp(`data-tab="${tab}"`),`底部缺少 ${tab} 一级入口`);
...

{
  ...
  for(const width of [320,390,844,1292])assert.ok(Math.abs(a.treeWidthFitZoom(width,L.W)*L.W-width)<.001,'默认缩放必须按整行宽度铺满，而不是把整棵长树缩到一屏');assert.ok(L.H>L.W*3,'文明阶段必须纵向展开');
  ...
  Object.entries(a.TECHS).forEach(([id,t])=>(t.req||[]).forEach(req=>{
    assert.ok(L.pos[id].y>L.pos[req].y+72,req+' → '+id+' 必须保持从上往下');
  }));
...
{
  const js=fs.readFileSync(__dirname+'/game.js','utf8'),ui=fs.readFileSync(__dirname+'/ui-system.css','utf8');
  assert.match(js,/tn-state">可研究/,'材料满足的可研究科技必须显示明确状态文字');assert.match(js,/tn-complete[\s\S]{0,100}已研究/,'完成科技必须显示勾选和已研究文字');
  assert.match(ui,/\.tnode\.ready\.can \.tn-card \{[^}]*border-width:2px[^}]*tech-researchable-pulse/,'可研究节点必须使用高亮青色脉冲态');assert.match(ui,/\.tnode\.max \.tn-card \{[^}]*#3fb87c[^}]*#123426/,'已研究节点必须使用稳定绿色完成态');assert.match(ui,/\.tnode\.max\.sel \.tn-card[\s\S]{0,180}#78e8ae/,'选中已研究节点时不得重新变回可研究的青色');
}
...
{
  const s=reset();s.player.equip.weapon='crowbar';s.player.equip.implant='lsChip';s.player.hp=50;a.startCombat('rat');s.combat.distNow=1;s.combat.hp=1;sandbox.Math.random=()=>0;
  a.playerAttack();assert.equal(s.combat,null,'测试攻击必须以击杀结束战斗');assert.ok(s.player.hp>50,'嗜血芯片必须在击杀一击造成伤害后正常恢复生命');
}

{
  const s=reset(); assert.equal(a.totalAtk(),13,'撬棍的入门战力增幅应生效，同时让前期敌人至少需要两次攻击');
...
}
{
  ...
  assert.equal(nodes.filter(n=>hasClass(n,'skill-browser')).length,1,'技能主画面只创建战技库，不保留三个分类页');assert.equal(nodes.filter(n=>hasClass(n,'skill-projection')).length,1,'所选能力只占据一个投影舞台');assert.equal(nodes.filter(n=>hasClass(n,'skill-inspector')||hasClass(n,'skill-effect-comparison')).length,0,'训练表单不放在主画面');
  assert.equal(nodes.filter(n=>hasClass(n,'skill-assign-slot')).length,3,'底部动作区必须直接提供三个战斗槽');assert.equal(nodes.filter(n=>String(n.className).includes('skill-category-tabs')).length,0,'移除三分类标签');assert.equal(nodes.filter(n=>hasClass(n,'skill-scene-entry')).length,2,'自动能力与精通为场景上的独立浮层入口');
  ...
  const catalogue=nodes.find(n=>hasClass(n,'skill-catalogue-toggle'));assert.ok(catalogue,'未来解锁路径必须保留在显示未学开关');catalogue.onclick();assert.equal(s.skillCatalogue,true);assert.ok(cards.filter(n=>!n.hidden).length>1);assert.equal(active.children[0].children,cards,'图鉴开关只过滤原节点，不重建技能库');
}
{
  ...
  const [stage,hero,browser,gameActions]=box.children,cards=browser.children[0].children,detail=a.renderSkillDetail('heavy',()=>box._refreshSkillPanel()),footer=new FakeElement();detail._mountActions(footer);const actions=footer.children.find(n=>hasClass(n,'skill-assignment-slots'));
  assert.equal(actions.children.length,3,'固定底部直接提供三个槽位，不需要装配模式再确认');actions.children[2].onclick();assert.equal(s.skillSlots[2],'heavy');assert.equal(actions.children[2].children[2].textContent,'点击卸下');actions.children[2].onclick();assert.equal(s.skillSlots[2],null,'再次点击明确标记的卸下位置必须真正卸下');
  assert.equal(box.children[0],stage);assert.equal(box.children[1],hero);assert.equal(box.children[2],browser);assert.equal(box.children[3],gameActions);assert.equal(browser.children[0].children,cards,'装配和卸下只更新原节点文字');
  assert.equal(s.skillView,'active');assert.equal(box.children.length,4,'不在后台保留自动和精通两个分类页');
}
{
  const s=reset();s.meta.careers.life={id:'salvager',level:3,xp:0};s.skills.salvageSense.prof=20;s.skills.fieldSorting.prof=10;s.skillView='auto';s.skillSelected='salvageSense';const box=a.renderSkillBrowser('auto',()=>{},{catalogue:false,remember:false}),nodes=[];(function walk(node){nodes.push(node);(node.children||[]).forEach(walk);})(box);const card=nodes.find(n=>hasClass(n,'skill-library-card')&&n.dataset.skill==='salvageSense'),detail=a.renderSkillDetail('salvageSense',()=>{}),footer=new FakeElement();detail._mountActions(footer,()=>{});
  assert.equal(card.children[1].children[1].textContent,'Lv2','自动分类图标必须显示等级');assert.match(detail.children[1].children[0].children[1].textContent,/采集 \+15%/,'当前栏必须展示真实效果');assert.match(detail.children[1].children[1].children[1].textContent,/采集 \+18%/,'下级栏必须使用真实增长公式');assert.match(detail.children[0].children[1].children[2].textContent,/已自动生效 · 熟练度 0 \/ 10/);assert.equal(footer.children.some(n=>hasClass(n,'skill-assignment-slots')),false,'生活被动不得出现装配按钮');
}
{
  const s=reset();s.charView='careers';s.careerMainSelected='bulwark';s.meta.careers.main={id:'vanguard',level:3,xp:12};s.meta.careers.life={id:'noviceCollector',level:2,xp:7};const box=new FakeElement(),nodes=[];a.renderCharPanel(box);(function walk(node){nodes.push(node);(node.children||[]).forEach(walk);})(box);
  assert.equal(s.careerView,'current');assert.equal(nodes.find(n=>hasClass(n,'career-identity')).dataset.career,'vanguard','当前身份来自真实职业，不读取过期路线预览');
  const markup=nodes.map(n=>n.innerHTML||'').join(' ');
  assert.match(markup,/脉冲游骑/);assert.doesNotMatch(markup,/舰盾卫士|裂隙猎手/,'未就任主战不在个人职业面板中出现');
  assert.match(markup,/职业经验 12 \/ 90/);assert.match(markup,/\+8<\/em> <em>\+30%/,'固定攻击成长与百分比加成按实际 Lv3 合并展示');
  assert.equal(nodes.filter(n=>hasClass(n,'career-identity')).length,1);assert.equal(nodes.filter(n=>hasClass(n,'career-view-tabs')||hasClass(n,'rpg-route-selector')).length,0);
  const specialties=nodes.filter(n=>hasClass(n,'career-specialty'));assert.equal(specialties.length,3);assert.deepEqual(specialties.map(n=>n.disabled),[false,true,true]);
  assert.equal(typeof specialties[0].onclick,'function');assert.ok(!specialties[1].onclick&&!specialties[2].onclick,'未激活的副职槽没有操作回调');
}
{
  const s=reset();s.careerView='life';s.meta.careers.life=[{id:'noviceCollector',level:1,xp:0}];const before=JSON.stringify([s.meta.careers,s.inv]),box=new FakeElement();a.renderCareerPanel(box);
  assert.equal(s.careerView,'current','旧生活分页状态回到统一职业页');assert.equal(box.children[0].dataset.career,'unassigned');assert.equal(box.children[1].children[0].children.length,3);
  assert.equal(JSON.stringify([s.meta.careers,s.inv]),before,'查看职业不修改身份、等级或材料');assert.equal(s.meta.careers.main,null,'查看副职业不授予主战身份');
  const rack=box.children[1].children[0];assert.match(rack.children[0].innerHTML,/入门拾荒者/);assert.equal(rack.children[0].disabled,false);assert.equal(rack.children[1].disabled,true);
}
{
  const s=reset();s.charView='careers';s.flags.job_vanguard_qualified=true;const box=new FakeElement(),nodes=[];a.renderCharPanel(box);(function walk(node){nodes.push(node);(node.children||[]).forEach(walk);})(box);
  const markup=nodes.map(n=>n.innerHTML||'').join(' ');assert.match(markup,/幸存者/);assert.match(markup,/老乔/);assert.doesNotMatch(markup,/脉冲游骑|舰盾卫士|裂隙猎手/,'有候选资格也不能假装已经就职');
  assert.equal(nodes.filter(n=>hasClass(n,'career-specialty')&&n.disabled).length,3);assert.equal(s.meta.careers.main,null);
}
{
...
}
{
  ...
  a.QUESTS.filter(q=>q.type==='search').forEach(q=>{const events=a.AREA_EVENTS[q.target]||[];assert.ok(events.length>=q.count,`调查任务 ${q.id} 的固定线索少于要求数量`);assert.equal(events.slice(q.count-1).some(event=>event.flag===q.targetFlag),true,`调查任务 ${q.id} 的最终线索不能产出完成旗标 ${q.targetFlag}`);});
}
{
  ...
  s.player.location='layer5';s.areaSearch.layer5=3;s.flags.patrolRecordsComplete=true;a.syncQuestProgress(false);assert.equal(a.questState('patrol'),'done');assert.equal(a.questState('bridge'),'active');
...
}
{
  ...
  assert.equal(a.questSearchCount({id:'faultAudit',target:'layer3'}),3,'接任务前已经确认的现场证据必须计入调查进度');
  a.syncQuestProgress(false);assert.equal(s.quests.faultAudit,'active','线索数量足够但最终证据旗标缺失时不能提前完成');
  s.flags.faultChainReady=true;a.syncQuestProgress(false);assert.equal(s.quests.faultAudit,'done','固定剧情事件与关键证据齐全后必须完成调查任务');
}
{
  const s=reset();s.quests.spore='done';a.activateAvailableQuests(false);assert.equal(s.quests.relay,'active');
  s.areaSearch.abyss=3;a.syncQuestProgress(false);assert.equal(s.quests.relay,'active','回声深井不能只有线索计数而跳过中继器剧情');assert.match(a.questProgress(a.QUESTS.find(q=>q.id==='relay')),/3\/3 条线索/);
...
}
{
  ...
  assert.equal(s.inv.signalCell,0,'信标战必须固定消耗一个信标电池');assert.equal(s.player.stamina,20,'进入任意信标层不得消耗体力');
}
{
  const floor1=a.beaconFloorSpec(1),floor11=a.beaconFloorSpec(11),floor100=a.beaconFloorSpec(100);
  assert.equal(floor1.manuals,1,'信标首层必须保底一册精通训练手册');
  assert.equal(floor11.manuals,2,'每十层必须再增加一册保底精通训练手册');
  assert.ok(floor100.mult>floor11.mult&&floor100.threat>floor11.threat,'无尽信标敌情与威胁必须随层数持续增长');
  assert.equal(floor1.cells,1);assert.equal(floor11.cells,1);assert.equal(floor100.cells,1,'不论层数，信标挑战都只能消耗一个信标电池');assert.equal(floor100.cost,0,'信标层数不得再次引入体力消耗');
  assert.ok(Object.keys(floor100.drops).length>Object.keys(floor1.drops).length,'高层必须解锁更多材料种类');
  assert.equal(a.beaconFloorSpec(5).equipmentGuaranteed,true,'每五层必须保底掉落装备');
}
{
  const s=reset();s.inv.signalCell=10;s.player.stamina=100;a.startBeacon(2);
  assert.equal(s.combat,null,'未通关前不得越层挑战');assert.equal(s.inv.signalCell,10,'越层失败不得扣除资源');
  const before=s.masteries.attackMastery;s.inv.masteryManual=6;a.useMasteryManual('attackMastery');
  assert.equal(s.masteries.attackMastery,before+1,'默认使用一本精通训练手册时必须提升一级');assert.equal(s.inv.masteryManual,5);
  assert.equal(a.useMasteryManual('attackMastery',4),true);assert.equal(s.masteries.attackMastery,before+5,'批量使用几本手册就必须提升几级');assert.equal(s.inv.masteryManual,1);
  assert.equal(a.useMasteryManual('attackMastery',2),false,'使用数量超过持有量时必须拒绝操作');assert.equal(s.masteries.attackMastery,before+5);assert.equal(s.inv.masteryManual,1);
}
{
  const s=reset();s.tutorial.complete=true;s.inv.masteryManual=150;s.siteSheet={kind:'masteryBook'};document.body.children=[];a.renderSiteSheet(new FakeElement());const backdrop=document.body.children[0],nodes=[];(function walk(node){nodes.push(node);(node.children||[]).forEach(walk);})(backdrop);const controls=nodes.find(n=>n.className==='mastery-book-controls'),input=nodes.find(n=>String(n.className).includes('mastery-book-quantity')),confirm=nodes.find(n=>String(n.className).includes('mastery-book-confirm')),choices=nodes.find(n=>n.className==='mastery-book-grid');assert.deepEqual(controls.children.map(n=>n.innerHTML),['-10','-1','', '+1','+10','+100']);controls.children.at(-1).click();assert.equal(input.value,'101','精通手册 +100 必须就地更新数量');assert.equal(document.body.children[0],backdrop,'调整精通手册数量不得重建弹层造成闪烁');choices.children[5].click();assert.match(confirm.innerHTML,/攻击精通 · Lv0 → Lv101[\s\S]*破限输出：攻击力 \+1%/,'必须先选择精通，再由独立确认按钮展示升级后的超限效果');
}
{
  const s=reset();Object.entries(a.MASTERIES).forEach(([id,m])=>{Object.keys(s.masteries).forEach(k=>s.masteries[k]=0);s.masteries[id]=100;const at100=a.masteryBonus(m.overStat);s.masteries[id]=101;assert.equal(a.masteryBonus(m.overStat)-at100,m.overPerLv,`${m.name} 在101级必须开始获得专属超限效果`);assert.doesNotMatch(a.masteryEffectText(id,101),/Lv101 解锁/,`${m.name} 的超限效果不能只有文案占位`);});assert.equal(a.masteryOverLevel(135),35,'100级后的每一级都必须计入超限成长');
}
{
  const s=reset();sandbox.Math.random=()=>.99;const reward=a.winBeacon(a.beaconFloorSpec(1));
  assert.equal(reward.manuals,1);assert.equal(s.inv.masteryManual,1);assert.equal(s.beaconMaxFloor,2,'通关当前最高层必须解锁下一层');
  assert.equal(s.siteSheet.kind,'beaconReward','信标奖励必须进入常驻结算面板');
  const repeat=a.winBeacon(a.beaconFloorSpec(1));assert.equal(s.beaconMaxFloor,2,'重复挑战旧层不得跳层解锁');assert.equal(repeat.unlocked,false);
  sandbox.Math.random=()=>.99;const bossReward=a.winBeacon(a.beaconFloorSpec(5));assert.ok(bossReward.equipment,'每五层装备保底不得受随机数影响');
}
{
  const s=reset();s.meta.built.beacon=true;s.meta.buildLevels.beacon=1;s.beaconMaxFloor=100;s.inv.signalCell=10;s.player.stamina=100;const box=new FakeElement();a.renderBuilding(box,'beacon');const all=[];(function walk(node){all.push(node);(node.children||[]).filter(child=>child&&typeof child==='object').forEach(walk);})(box);const scroller=all.find(node=>hasClass(node,'beacon-floor-scroll')),selected=all.find(node=>hasClass(node,'selected'));
  assert.ok(scroller,'高层信标必须使用页面内固定高度的纵向滚动选层框');assert.equal(scroller.children.length,100,'纵向选层框必须能选择全部已解锁层数');assert.match(scroller.children[0].innerHTML,/FLOOR 100/,'进入信标时最高层必须直接放在滚动列表顶部');assert.match(selected.innerHTML,/FLOOR 100/,'进入信标时必须默认选中最高层');assert.equal(scroller.children[2].onclick,undefined,'滑到目标层必须自动选中，不得要求再次点击层数');scroller.scrollTop=92;scroller.onscroll();assert.equal(s.beaconSelectedFloor,98,'纵向滚动停止后必须按停靠位置自动选择层数');assert.ok(all.some(node=>hasClass(node,'beacon-floor-preview'))&&all.some(node=>hasClass(node,'beacon-challenge')),'选层框下方必须固定展示掉落详情和出发按钮');
}
{
  ...
  const hp=s.combat.hp;a.useSkill('heavy'); assert.ok(s.combat===null||s.combat.hp<hp,'技能必须能实际造成伤害');
}
{
  const s=reset(); s.skills.heavy.prof=10; s.player.stamina=10; a.startCombat('rat'); s.combat.distNow=1;
  const hp=s.combat.hp;a.useSkill('heavy'); assert.equal(s.combat.hp,hp,'未装配的主动技能不得在战斗中使用');
...
}
{
  ...
  assert.equal(a.effectiveEconomyBonus('craftSavePct'),200);assert.equal(a.effectiveEconomyBonus('gatherPct'),400);assert.equal(a.effectiveEconomyBonus('minePct'),400);assert.equal(a.effectiveEconomyBonus('recyclePct'),400);assert.equal(a.effectiveEconomyBonus('gardenPct'),400,'生产精通必须按等级持续成长而不设置有效上限');
  ...
  const oldLevel=s.masteries.craftMastery;s.inv.ration=999;a.upgradeMastery('craftMastery');assert.equal(s.masteries.craftMastery,oldLevel+1,'生产精通达到旧上限后仍必须可以升级');assert.equal(a.masteryAtEffectiveCap('craftMastery'),false);assert.doesNotMatch(a.masteryEffectText('craftMastery',oldLevel),/上限/);
  s.masteries.attackMastery=10000;s.inv.crystal=2000;const before=s.inv.crystal;a.upgradeMastery('attackMastery');assert.equal(s.masteries.attackMastery,10001,'精通超过 Lv9999 后仍必须继续升级');assert.ok(s.inv.crystal<before,'超高等级精通必须继续消耗递增材料，不能出现免费升级');
}
{
...
}
{
  ...
  Object.entries(a.ENEMIES).forEach(([id,e])=>assert.ok(a.COMBAT_ERAS[e.era],`敌人 ${id} 必须归属固定装备时代`));
  const eraAverages=Object.keys(a.COMBAT_ERAS).map(Number).map(era=>{const rows=Object.values(a.ENEMIES).filter(e=>e.era===era&&!e.boss);return {era,hp:rows.reduce((n,e)=>n+e.hp,0)/rows.length,atk:rows.reduce((n,e)=>n+e.atk,0)/rows.length,def:rows.reduce((n,e)=>n+e.def,0)/rows.length};});
  for(let i=1;i<eraAverages.length;i++){const prev=eraAverages[i-1],next=eraAverages[i];assert.ok(next.hp>prev.hp&&next.atk>prev.atk&&next.def>prev.def,`装备时代 ${next.era} 的固定普通怪预算必须全面高于时代 ${prev.era}`);}
  ...
  a.BEACON.forEach((row,index)=>{assert.ok(row.mult>0&&row.threat>0&&row.cost===0&&row.cells===1&&row.manuals>0&&row.equipmentChance>0,`信标层 ${row.name} 的数值无效`);if(index){const prev=a.BEACON[index-1];assert.ok(row.mult>prev.mult&&row.threat>prev.threat&&row.equipmentChance>prev.equipmentChance,`信标层 ${row.name} 必须同步提高敌情、威胁与装备概率`);}});
...
}
{
  ...
  assert.ok(combatBand(10,'rifle','vest','warbot').hits>=3&&combatBand(10,'rifle','vest','warbot').hits<=5,'钢装节点的军事区普通怪应维持3-5回合交战');
  const boss=combatBand(10,'eblade','power','guardian');assert.ok(boss.hits>=14&&boss.hits<=20,'钢装节点首领必须保留足够回合完整展示机制');assert.ok(boss.taken>=15,'动力甲阶段首领每次命中仍必须造成有效净伤害');
}
{
  ['surface','ark','depth','orbit','ashMoon','verdant','silent'].forEach(region=>{const sites=a.WORLD_REGIONS[region].locations.map(id=>[id,a.LOCATIONS[id]]).filter(([,loc])=>loc.boss);assert.ok(sites.length>0,region+' 探索区域必须拥有至少一个固定首领地点');sites.forEach(([,loc])=>{assert.ok(a.ENEMIES[loc.boss]&&a.ENEMIES[loc.boss].boss,loc.boss+' 必须是真正的首领单位');assert.equal(a.ENEMIES[loc.boss].bossFlag,loc.bossFlag,'地点与首领必须共用唯一击败状态');});});
  const s=reset();s.player.location='coalRift';s.exploreCount.coalRift=4;const marker=a.fieldMapMarkers('coalRift').find(row=>row.kind==='boss');assert.ok(marker&&marker.target==='riftMatriarch','地表首领必须在达到调查深度后作为独立地图节点出现');
}
{
...
}
{
  ...
  assert.equal(a.SKILLS.fieldRepair.name,'装配流水线');assert.equal(a.SKILLS.fieldRepair.type,'passive');assert.equal(a.craftHours(10),8,'入门学徒的装配流水线必须缩短批量制造耗时');assert.equal(!!s.meta.damaged.work,true,'被动制造技能不得再充当设施修理按钮');
  s=reset();s.meta.careers.life={id:'noviceGrower',level:1,xp:0};s.meta.built.garden=true;s.meta.buildLevels.garden=1;s.player.stamina=10;s.inv.wood=2;a.plantGarden(0,'nutrient');const readyAt=s.gardenPlots[0].readyAt;
  assert.equal(a.performSporeBoost(0),true);assert.equal(s.gardenPlots[0].readyAt,readyAt-2);assert.equal(s.player.stamina,8);assert.equal(s.skills.sporeBoost.prof,11);assert.equal(a.performSporeBoost(0),true,'催生孢子应可对同一培养槽反复使用，不受每日次数限制');assert.equal(s.player.stamina,6);
  s.gardenPlots[0].readyAt=s.time+8;const garden=new FakeElement(),gardenNodes=[];a.renderBuilding(garden,'garden');(function walk(node){gardenNodes.push(node);(node.children||[]).forEach(walk);})(garden);const beds=gardenNodes.filter(n=>hasClass(n,'garden-bed')),boosts=gardenNodes.filter(n=>hasClass(n,'garden-bed-skill'));assert.equal(boosts.length,beds.length,'每个培养块都必须拥有自己的培育师技能入口');const activeBoost=boosts.find(n=>hasClass(n,'active'));assert.ok(activeBoost,'生长中的培养块必须显示可用的催生孢子按钮');assert.match(activeBoost.innerHTML,/催生孢子[\s\S]*体力 2[\s\S]*推进 2 小时/,'培养块技能必须直接显示消耗和推进量');const uiReadyAt=s.gardenPlots[0].readyAt,gardenShellBefore=gardenNodes.find(n=>hasClass(n,'garden-workbench')),activeBedBefore=activeBoost.parentNode;activeBoost.click();assert.equal(s.gardenPlots[0].readyAt,uiReadyAt-2,'点击对应培养块上的技能必须只推进该块生长');assert.ok(gardenShellBefore.parentNode,'培养块技能结算不能重建整个菌圃工作台');assert.equal(activeBedBefore.parentNode,null,'培养块技能结算必须只替换被操作的培养块');
...
}
{
  ...
  s.meta.built.garden=true;s.meta.buildLevels.garden=1;s.inv.wood=1;a.plantGarden(0,'nutrient');s.time+=4;a.harvestGarden();assert.equal(a.careerRecord('life','biologist').xp,6,'菌圃经验只能进入培育路线');
...
}
{
  ...
  assert.equal(nodes.filter(n=>hasClass(n,'career-identity')).length,1,'只显示当前主战职业');assert.match(markup,/脉冲游骑/);
  const choices=nodes.filter(n=>hasClass(n,'career-specialty'));assert.equal(choices.length,3);assert.ok(choices.every(n=>!n.disabled),'三个已取得副职业的详情同时可用');assert.deepEqual(choices.map(n=>n.dataset.career),['salvager','fabricator','biologist']);
}
{
...
{
  let s=reset();a.startCombat('exp');s.combat.distNow=1;let rolls=[.99,0];sandbox.Math.random=()=>rolls.shift();a.resolveEnemyAction();assert.equal(s.player.infected,true,'没有隔离膜时感染型敌人仍应造成感染');
  s=reset();s.inv.bioSuit=1;a.startCombat('exp');s.combat.distNow=1;rolls=[.99,0];sandbox.Math.random=()=>rolls.shift();a.resolveEnemyAction();assert.equal(s.player.infected,false,'持有生物隔离膜组件时必须阻止战斗造成的新感染');
}
{
  ...
  const characterAtk=vm.runInContext('baseAtk()',sandbox),expectedDelta=Math.round(characterAtk*(1+a.ITEMS.vacuumCarbine.powerPct/100)+a.ITEMS.vacuumCarbine.atk)-Math.round(characterAtk*(1+a.ITEMS.crowbar.powerPct/100)+a.ITEMS.crowbar.atk);assert.equal(stellar-salvage,expectedDelta,'战力增幅必须放大人物攻击，武器自身攻击只能作为独立固定值相加');assert.ok(a.ITEMS.vacuumCarbine.atk>=800&&a.ITEMS.vacuumCarbine.powerPct>=140,'星际武器的独立攻击与人物战力增幅必须足以匹配后期成长');assert.equal(a.ITEMS.vacuumCarbine.grade,'stellar');assert.equal(a.EQUIPMENT_GRADES.stellar.material,'星舰铱合金 / 活体复材');
...
{
  let s=reset();a.startCombat('rat');assert.deepEqual([s.combat.maxHp,s.combat.atk,s.combat.def],[a.ENEMIES.rat.hp,a.ENEMIES.rat.atk,a.ENEMIES.rat.def],'前期敌人必须使用固定数值');
  s=reset();s.player.level=75;a.GENE_NODES.forEach(g=>s.meta.geneNodes[g.id]=true);s.player.equip.weapon='vacuumCarbine';a.startCombat('gateCustodian');assert.equal(Object.hasOwn(s.combat,'adaptiveThreat'),false);assert.deepEqual([s.combat.maxHp,s.combat.atk,s.combat.def],[a.ENEMIES.gateCustodian.hp,a.ENEMIES.gateCustodian.atk,a.ENEMIES.gateCustodian.def],'远航终局敌人也必须保持固定数值，不能读取玩家基因或装备');
}
{
...
}
{
  ...
  a.syncQuestProgress(false);assert.equal(s.quests.findAtuoUnderworks,'done');assert.equal(s.quests.underworksCache,'active');s.areaSearch.underworks=2;a.syncQuestProgress(false);assert.equal(s.quests.underworksCache,'active','维修线索齐全但运输轨旗标未确认时不能提前完成');s.flags.underworksCacheMapped=true;a.syncQuestProgress(false);assert.equal(s.quests.underworksCache,'done');
