...

{
  ...
  assert.match(androidBuild,/versionCode 7/,'启动更新门 APK 必须提升安装版本');
  assert.match(androidBuild,/versionName "0\.5\.0"/,'启动更新门 APK 必须展示新的应用版本');
  assert.match(androidBuild,/SHELL_VERSION", "6"/,'Wi-Fi 更新判断必须升级原生外壳协议版本');
  assert.match(androidBuild,/BUNDLED_BUILD", "15L"/,'最新 APK 必须内置启动舱门资源 build 15');
  ...
  const bundleUpdater=fs.readFileSync(path.join(__dirname,'..','android','app','src','main','java','com','exkalib','abyssalecho','BundleUpdater.java'),'utf8');assert.match(bundleUpdater,/onUpdateAvailable\(UpdateInfo update\)[\s\S]*downloadAndInstall\(UpdateInfo update/,'更新器必须先报告发现更新，再由网络策略决定是否下载');assert.match(bundleUpdater,/listener\.onProgress\(progress\)/,'资源下载必须把真实百分比回传开屏');
