...

{
  ...
  assert.match(html,/style\.css\?v=[^"']*cloudsave2[\s\S]*game\.js\?v=[^"']*cloudsave3/,'云存档脚本与样式必须同时更新缓存版本，避免 Safari 混用旧资源');
  ...
  assert.match(androidBuild,/SHELL_VERSION", "4"/,'安卓云存档网络桥接必须升级原生外壳协议版本');
  assert.match(androidBuild,/BUNDLED_BUILD", "12L"/,'最新 APK 必须内置云存档客户端资源 build 12');
  ...
  const mainActivity=fs.readFileSync(path.join(__dirname,'..','android','app','src','main','java','com','exkalib','abyssalecho','MainActivity.java'),'utf8');assert.match(mainActivity,/@JavascriptInterface[\s\S]{0,120}cloudRequest\(/,'APK 必须把云存档请求桥接给网页资源');
  const cloudStore=fs.readFileSync(path.join(__dirname,'..','netlify','lib','postgres-cloud-save.mjs'),'utf8');assert.match(cloudStore,/SELECT revision, updated_at[\s\S]*FOR UPDATE[\s\S]*save_conflict/,'Netlify Database 必须以行锁和基础版本拒绝旧档覆盖');assert.match(cloudStore,/abyss_cloud_save_history[\s\S]*HISTORY_LIMIT/,'Netlify Database 必须保留最近20代历史备份');
  const cloudMigration=fs.readFileSync(path.join(__dirname,'..','netlify','database','migrations','20260902090000_create_abyss_cloud_saves.sql'),'utf8');assert.match(cloudMigration,/CREATE TABLE IF NOT EXISTS abyss_cloud_saves[\s\S]*CREATE TABLE IF NOT EXISTS abyss_cloud_save_history/,'Netlify 部署必须自动创建云存档表与历史表');
  const netlifyConfig=fs.readFileSync(path.join(__dirname,'..','netlify.toml'),'utf8');assert.match(netlifyConfig,/from = "\/api\/cloud-save"[\s\S]*to = "\/\.netlify\/functions\/cloud-save"/,'云存档接口必须由同一 Netlify 项目的 Function 承载');assert.doesNotMatch(netlifyConfig,/59\.110\.144\.30/,'Netlify 不得继续代理旧服务器');
  ...
  assert.match(source,/baseRevision:bindingAtStart\.revision[\s\S]*error\.status===409/,'自动同步必须携带基础版本并显式处理云端冲突');
  assert.match(source,/initial&&!localSaveWasLoaded[\s\S]{0,260}installSaveAndReload\(result\.save/,'已绑定云档但本地档缺失时必须恢复云端，不能把新空档反向覆盖服务器');
