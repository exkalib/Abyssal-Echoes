...

{
  ...
  assert.match(androidBuild,/versionCode 6/,'离线优先 APK 必须提升安装版本，覆盖旧服务器版本');
  assert.match(androidBuild,/versionName "0\.4\.1"/,'离线优先 APK 必须展示新的应用版本');
  assert.match(androidBuild,/SHELL_VERSION", "5"/,'离线优先 APK 必须升级原生外壳协议版本');
  assert.match(androidBuild,/BUNDLED_BUILD", "14L"/,'最新 APK 必须内置手动迁移客户端资源 build 14');
  ...
  const netlifyConfig=fs.readFileSync(path.join(__dirname,'..','netlify.toml'),'utf8');assert.doesNotMatch(netlifyConfig,/\.netlify\/functions\/cloud-save|59\.110\.144\.30/,'迁移接口不得暴露可绕过限流的默认 Function 路径，也不得回退旧服务器');
  ...
  assert.match(source,/只在点击按钮时联网检查；启动游戏不会访问更新服务器/,'设置页必须准确说明 APK 更新检查也是手动触发');
  assert.doesNotMatch(source,/启动时会自动检查/,'离线优先 APK 不得继续显示自动更新检查文案');
