...
(async()=>{
  ...
  try{
    for(const width of [360,390,520,768]){
      ...
      await page.evaluate(()=>{window.rpgRefs={hero:document.querySelector('.skill-console-head'),list:document.querySelector('.skill-library-grid'),inspector:document.querySelector('.skill-inspector')};});
      await page.locator('.skill-library-card[data-skill="heavy"]').click();
      assert.equal(await page.evaluate(()=>window.rpgRefs.hero===document.querySelector('.skill-console-head')&&window.rpgRefs.list===document.querySelector('.skill-library-grid')&&window.rpgRefs.inspector===document.querySelector('.skill-inspector')),true,'skill selection preserves mounted library and inspector');
      assert.equal(await page.locator('.skill-detail-panel').count(),1);assert.equal(await page.locator('.skill-thumb-sheet').count(),0);await noOverflow();
      ...
      await page.locator('.skill-category-tabs button').nth(1).click();await page.locator('.skill-browser:visible .skill-library-card').first().click();assert.match(await page.locator('.skill-detail-panel').innerText(),/现场|生活|副职业|自动/);await noOverflow();
      ...
      assert.equal(await page.locator('.career-path-card').count(),1);assert.equal(await page.locator('.career-view-tabs button').count(),2);assert.equal(await page.locator('.rpg-route-selector.selected').getAttribute('data-career'),'bulwark');await noOverflow();
      await page.locator('.career-view-tabs button').nth(0).click();
      ...
      await page.locator('.career-view-tabs button').nth(1).click();assert.equal(await page.locator('.rpg-route-selector.learned').count(),3);await noOverflow();
