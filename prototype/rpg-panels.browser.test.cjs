...
(async()=>{
  ...
  try{
    for(const width of [360,390,520,768]){
      ...
      assert.equal(await page.locator('.skill-detail-panel').count(),1);await noOverflow();await page.locator('.skill-sheet-close').click();
      ...
      await page.locator('.skill-category-tabs button').nth(1).click();await page.locator('.skill-browser:visible .skill-library-card').first().click();assert.match(await page.locator('.skill-detail-panel').innerText(),/现场|生活|副职业|自动/);await noOverflow();await page.locator('.skill-sheet-close').click();
      ...
      await page.locator('.bag-detail-back').click();
