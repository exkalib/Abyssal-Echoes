...
const ITEMS = {
  ...
  // 武器
  crowbar:{name:'撬棍',type:'equip',slot:'weapon',weaponType:'melee',staminaCost:3,atk:3,powerPct:10,range:1,icon:'🔧'},
  knife:{name:'铁刀',type:'equip',slot:'weapon',weaponType:'melee',staminaCost:2,atk:20,powerPct:25,range:1,crit:5,icon:'🔪'},
  blade:{name:'合金刃',type:'equip',slot:'weapon',weaponType:'melee',staminaCost:3,atk:48,powerPct:40,range:1,critDmg:25,icon:'🗡️'},
  eblade:{name:'能量战刃',type:'equip',slot:'weapon',weaponType:'melee',staminaCost:2,atk:110,powerPct:60,range:1,pen:18,icon:'⚔️'},
  pistol:{name:'磁轨手枪',type:'equip',slot:'weapon',weaponType:'ranged',ammo:'ammo',ammoCost:1,atk:32,powerPct:30,range:4,crit:8,icon:'🔫'},
  ...
  workGloves:{name:'加固作业手套',type:'equip',slot:'hands',def:3,hit:3,icon:'▤',desc:'以钢丝和缓冲层保护手掌，改善废墟作业与基础武器操控。'},
  servoGauntlet:{name:'伺服动力臂铠',type:'equip',slot:'hands',atk:18,def:6,hit:5,icon:'▥',desc:'微型伺服束放大握持与后坐力控制，适配合金和能量武器。'},
  // 胸
  ...
  phaseGrip:{name:'相位约束臂环',type:'equip',slot:'hands',atk:64,def:14,crit:10,pen:12,icon:'◈',desc:'在接触瞬间校准局部相位，降低重力与真空武器的操控损耗。'},
  ...
  vacuumCarbine:{name:'真空磁束卡宾枪',type:'equip',slot:'weapon',weaponType:'ranged',ammo:'weaponCell',ammoCost:2,atk:820,powerPct:140,range:10,pen:52,hit:18,mechDamagePct:25,bossDamagePct:18,icon:'⌁'},
  ...
  masteryManual:{name:'精通训练手册',type:'masteryBook',icon:'▣',desc:'从背包使用后选择一项精通，直接提升 1 级；不消耗导师升级材料。'},
  // 结局奖杯
...
const EQUIPMENT_GRADES = {
  ...
  field:{name:'场能级',material:'钛合金 / 超导线圈',tone:'紫',rule:'武器以战力增幅强化人物攻击，武器自身攻击保持独立'},
  quantum:{name:'量子级',material:'量子核心 / 纳米机群',tone:'金',rule:'具有目标类型加成或规则型防护'},
  stellar:{name:'星际级',material:'星舰铱合金 / 活体复材',tone:'红',rule:'高额独立装备数值，并针对首领或异星单位'},
};
...
/* ================= 精通(100级后每级同时强化专属超限效果) ================= */
const MASTERIES = {
  gatherMastery:  {name:'采集精通', stat:'gatherPct',    perLv:10, route:'prod', npc:'老乔',  desc:'采集产量 +{v}%',overStat:'stMax',overPerLv:1,overDesc:'超限负载：体力上限 +{v}'},
  mineMastery:    {name:'挖矿精通', stat:'minePct',      perLv:10, route:'prod', npc:'老周',  desc:'矿产获取 +{v}%',overStat:'def',overPerLv:1,overDesc:'岩层适应：防御力 +{v}'},
  recycleMastery: {name:'拆解精通', stat:'recyclePct',   perLv:10, route:'prod', npc:'老周',  desc:'拆解产量 +{v}%',overStat:'gatherPct',overPerLv:3,overDesc:'材料辨识：采集产量额外 +{v}%'},
  craftMastery:   {name:'制造精通', stat:'craftSavePct', perLv:10, route:'prod', npc:'阿珍',  desc:'材料返还 +{v}%',overStat:'smeltPct',overPerLv:3,overDesc:'热场复用：熔炼产量 +{v}%'},
  gardenMastery:  {name:'培育精通', stat:'gardenPct',    perLv:10, route:'prod', npc:'陈嫂',  desc:'菌圃收获 +{v}%',overStat:'bioGatherPct',overPerLv:3,overDesc:'共生诱导：生物采集 +{v}%'},
  attackMastery:  {name:'攻击精通', stat:'atk',          perLv:1,  route:'combat',npc:'哈里斯',desc:'攻击力 +{v}',overStat:'atkPct',overPerLv:1,overDesc:'破限输出：攻击力 +{v}%'},
  defenseMastery: {name:'防御精通', stat:'def',          perLv:1,  route:'combat',npc:'小唐',  desc:'防御力 +{v}',overStat:'hpPct',overPerLv:1,overDesc:'装甲共振：生命上限 +{v}%'},
...
const MASTERY_ROUTES = {
  combat:[ // 战斗系
    ...
    {lo:100,hi:Infinity,mat:{crystal:1},scale:(lv)=>5+Math.floor((lv-100)/10)},
  ],
  prod:[ // 生产系
    {lo:1,hi:10,mat:{scrap:1},     scale:(lv)=>2+Math.floor((lv-1)/2)},
...
function masteryOverLevel(lv){return Math.max(0,Math.floor(Number(lv)||0)-100);}
function masteryBonus(stat){ let n=0; for(const k in MASTERIES){const m=MASTERIES[k],lv=masteryLv(k);if(m.stat===stat)n+=m.perLv*lv;if(m.overStat===stat)n+=m.overPerLv*masteryOverLevel(lv);} return n; }
function effectiveEconomyBonus(stat){return Math.max(0,jobBonus(stat));}
function masteryAtEffectiveCap(){return false;}
function masteryEffectText(k,lv){const m=MASTERIES[k],base=m.desc.replace('{v}',m.perLv*lv),over=masteryOverLevel(lv);return base+' · '+(over?m.overDesc.replace('{v}',Math.round(m.overPerLv*over*10)/10):'超限效果：Lv101 解锁');}
...
function upgradeMastery(k,refresh){ const cost=masteryCost(k); for(const[mat,n] of Object.entries(cost)){if((state.inv[mat]||0)<n){log('材料不足：'+costText(cost)+'。','warn');return;}}
  for(const[mat,n] of Object.entries(cost)) state.inv[mat]-=n; state.masteries[k]++; markInlineChange('mastery',k);log('✦ '+MASTERIES[k].name+' 升至 Lv'+state.masteries[k]+'!','good',{toast:false}); divider(); if(refresh)refresh();else render(); }
function useMasteryManual(k,quantity,refresh){
  const count=Math.max(1,Math.floor(Number(quantity)||1));if(!MASTERIES[k]||(state.inv.masteryManual||0)<count)return false;
  state.inv.masteryManual-=count;state.masteries[k]=(state.masteries[k]||0)+count;state.siteSheet=null;markInlineChange('mastery',k);log('研读精通训练手册×'+count+'：【'+MASTERIES[k].name+'】提升至 Lv'+state.masteries[k]+'。','good',{toast:false});if(refresh)refresh();else render();return true;
}
...
/* ================= 敌人 ================= */
const ENEMIES = {
  cleaner:{name:'清洁机器人',hp:24,atk:7,def:2,spd:3,dist:2,range:1,threat:6,mech:true,drops:{scrap:[1,2]}},
  rat:{name:'变异鼠',hp:18,atk:8,def:1,spd:6,dist:2,range:1,threat:5,drops:{ration:[0,1],cloth:[0,1],mutantMeat:[0,1]}},
...
const ENEMY_ERA = {
  ...
  radSpider:2,exp:2,sporeling:2,turret:2,warbot:2,guardian:2,arsenalWarden:2,
  echoBeast:3,echoLeviathan:3,
  scrapDrone:4,vacuumInterceptor:4,relayCorsair:4,
  lunarCrawler:5,massDriverAI:5,outpostRaid:5,
  xenoStalker:6,livingBulwark:6,planetaryCrown:6,
  phaseSentinel:7,gateCustodian:7,
};
Object.entries(ENEMY_ERA).forEach(([id,era])=>ENEMIES[id].era=era);
const ENEMY_PORTRAIT_ROOT='assets/enemy-portraits-v1/';
function enemyPortraitId(id){return id==='beacon'?'echoBeast':id;}
function enemyPortraitSrc(id){return ENEMY_PORTRAIT_ROOT+enemyPortraitId(id)+'.png?v=1';}
...
const LOCATIONS = {
  ...
  coalRift:{name:'碳脉裂谷',zone:'地表',profile:'mine',icon:'⬡',boss:'riftMatriarch',bossFlag:'surfaceBossDown',bossSearch:4,desc:'撞击暴露了浅层碳脉，也震开了通往船底维修井的兽穴。更深处有一头母兽控制着整片巢群。',enemies:['burrower','beast'],loot:{coal:.78,stone:.36,copperScrap:.25}},
  ...
  droneHangar:{name:'自律机库',zone:'隐秘',profile:'facility',icon:'⌁',needTech:'auto_3',boss:'arsenalWarden',bossFlag:'arkBossDown',bossSearch:4,resourceSite:{label:'无人机构件库',yield:['ecomp','core','titaniumOre']},desc:'军事区侧翼封存着一支失去指挥链的工程无人机群。兵工封锁机守着完整的高强度机体材料。',enemies:['turret','warbot'],loot:{ecomp:.68,core:.42,titaniumOre:.38,steel:.4}},
  ...
  signal:{name:'地下信号源',zone:'地下',profile:'archive',icon:'◌',boss:'echoLeviathan',bossFlag:'depthBossDown',bossSearch:3,desc:'这里并非天然洞穴，而是一座比方舟更古老的幸存者信标。巨型回响生命盘踞在信标主腔。',enemies:['echoBeast'],loot:{crystal:.62,core:.35}},
...
/* 商路随真实地图探索扩充。装备限持一件；回收价会在报价层再次按折后买价封顶，避免声望套利。 */
const SETTLEMENT_SHOP_TIERS = [
  {id:1,min:0,name:'聚居地基础供货',code:'LOCAL STOCK'},
  {id:2,min:6,name:'地表商路',code:'SURFACE ROUTE'},
  {id:3,min:14,name:'深区商路',code:'DEEP ROUTE'},
  {id:4,min:24,name:'核心交换网',code:'CORE EXCHANGE'},
];
const SETTLEMENT_SHOP_CATEGORIES = {
  all:{name:'全部',code:'ALL'},
  material:{name:'物资',code:'SUPPLY'},
  equipment:{name:'装备',code:'GEAR'},
  special:{name:'特殊',code:'SPECIAL'},
};
const SETTLEMENT_SHOP = {
  cloth:{tier:1,category:'material',shelf:'公共补给线',buy:{crystal:1,amount:3},sell:{amount:5,crystal:1}},
  ration:{tier:1,category:'material',shelf:'公共补给线',buy:{crystal:1,amount:2},sell:{amount:4,crystal:1}},
  coal:{tier:1,category:'material',shelf:'老周 · 回收仓',buy:{crystal:1,amount:4},sell:{amount:7,crystal:1}},
  ecomp:{tier:1,category:'material',shelf:'阿珍 · 零件柜',buy:{crystal:2,amount:1},sell:{amount:3,crystal:2}},
  copperScrap:{tier:1,category:'material',shelf:'老周 · 回收仓',buy:{crystal:2,amount:2},sell:{amount:5,crystal:2}},
  ...
  ingot:{tier:2,category:'material',shelf:'地表商队 · 工业料架',buy:{crystal:3,amount:1},sell:{amount:4,crystal:2}},
  ammo:{tier:2,category:'special',shelf:'哈里斯 · 巡逻军需',buy:{crystal:2,amount:10},sell:{amount:25,crystal:2}},
  serum:{tier:2,category:'special',shelf:'陈嫂 · 隔离药柜',buy:{crystal:5,amount:1},sell:{amount:2,crystal:2}},
  emp:{tier:2,category:'special',shelf:'阿珍 · 战术器材柜',buy:{crystal:6,amount:1},sell:{amount:2,crystal:2}},
  pistol:{tier:2,category:'equipment',shelf:'哈里斯 · 巡逻军械',unique:true,buy:{crystal:9,amount:1},sell:{amount:1,crystal:3}},
  vest:{tier:2,category:'equipment',shelf:'哈里斯 · 巡逻军械',unique:true,buy:{crystal:8,amount:1},sell:{amount:1,crystal:3}},
  scope:{tier:2,category:'equipment',shelf:'阿珍 · 精密装备柜',unique:true,buy:{crystal:9,amount:1},sell:{amount:1,crystal:3}},

  steel:{tier:3,category:'material',shelf:'深区商队 · 合金料架',buy:{crystal:4,amount:1},sell:{amount:4,crystal:3}},
  biocore:{tier:3,category:'material',shelf:'陈博士 · 样本交换柜',buy:{crystal:4,amount:1},sell:{amount:3,crystal:2}},
  blade:{tier:3,category:'equipment',shelf:'阿珍 · 合金装备柜',unique:true,buy:{crystal:13,amount:1},sell:{amount:1,crystal:4}},
  rifle:{tier:3,category:'equipment',shelf:'哈里斯 · 深区军械',unique:true,buy:{crystal:16,amount:1},sell:{amount:1,crystal:5}},
  eshieldUnit:{tier:3,category:'equipment',shelf:'小唐 · 护盾器材柜',unique:true,buy:{crystal:16,amount:1},sell:{amount:1,crystal:5}},
  critCore:{tier:3,category:'equipment',shelf:'阿珍 · 战术模组柜',unique:true,buy:{crystal:14,amount:1},sell:{amount:1,crystal:4}},
  pierceBook:{tier:3,category:'special',shelf:'哈里斯 · 战术档案',buy:{crystal:12,amount:1},sell:{amount:1,crystal:3}},
  ...
  power:{tier:4,category:'equipment',shelf:'核心交换网 · 重型防护',unique:true,buy:{crystal:24,amount:1},sell:{amount:1,crystal:8}},
...
const EQUIPMENT_RECYCLE_YIELDS={
  ...
  field:{titanium:1,superconductor:1},quantum:{nanites:1,quantumCore:1},stellar:{programmableMatter:1,starAlloy:1},
};
const BEACON_EQUIPMENT=[
...
const TECHS = {
  ...
  surv_7:{n:'再生医学',b:'生存医疗',era:6,hours:12,cost:{nanites:3,biocore:8,carbonComposite:3},req:['surv_6','make_7','make_10'],fac:'bioforge',un:['nanoMedkit','nanoSuit','nanoWeaveGloves'],bonus:{hp:45},desc:'让纳米机群参与组织修复，而不是只把伤势压回数值。'},
...
function geneGateChecks(g){const gate=g.gate||{},out=[];if(gate.level)out.push({ok:P().level>=gate.level,text:'角色等级 '+P().level+'/'+gate.level});if(gate.kills)out.push({ok:state.kills>=gate.kills,text:'本周目击杀 '+state.kills+'/'+gate.kills});if(gate.techs){const n=Object.keys(state.meta.techs||{}).filter(techKnown).length,need=state.meta.legacyTechGates&&gate.legacyTechs?gate.legacyTechs:gate.techs;out.push({ok:n>=need,text:'已研究科技 '+n+'/'+need});}if(gate.quest){const q=QUEST_BY_ID[gate.quest];out.push({ok:questDone(gate.quest),text:'任务【'+(q?(q.title||q.name):gate.quest)+'】'});}if(gate.playthrough)out.push({ok:state.meta.playthrough>=gate.playthrough,text:'周目 '+state.meta.playthrough+'/'+gate.playthrough});if(gate.endings){const n=(state.meta.endingsDone||[]).length;out.push({ok:n>=gate.endings,text:'不同结局 '+n+'/'+gate.endings});}return out;}
...
function unlockGeneNode(id){ const g=GENE_BY_ID[id]; if(!g||geneUnlocked(id))return; if(!(g.req||[]).every(geneUnlocked)){log('前置基因节点尚未解锁。','warn');return;} if(!geneGateReady(g)){log('基因表达条件尚未满足。','warn');return;} if(!canAfford(g.cost)){log('基因材料不足：'+costText(g.cost)+'。','warn');return;}
  payCost(g.cost); state.meta.geneNodes[id]=true; state.meta.gene=geneTier(); P().gene=state.meta.gene; state.geneSel=id; log('🧬 基因锁突破 → '+g.name+' · '+g.mutation,'good'); divider(); refreshGenePanel(); }
...
function freshState(keepMeta){
  ...
  const next = {
    ...
    defenses:[], gardenPlots:[], droneFleet:{scout:0,hauler:0,phase:0}, droneLastAt:0, rests:0, skills:{}, skillSlots:[null,null,null], skillSlotSel:0, skillView:'active', skillSelected:null, careerView:'current', charView:'overview', quests:{first_exit:'active'}, questStart:{}, flags:{}, areaSearch:{}, exploreCount:{}, discoveryThresholds:{}, fieldFogSeen:{}, resourceSites:{}, resourcePools:{}, investigationMisses:{}, discovered:{camp:true,outer:true,joeCamp:true}, dailyGather:{}, dailyLocation:{}, dailyFacility:{}, foodBuff:null, truthClaimed:null,
...
/* 战力增幅只放大已经结算完基因/职业/轮回的人物攻击；武器自身攻击与其他装备攻击均保持独立固定值。 */
function totalAtk(){ const combatMeal=(state.combat&&state.combat.foodAtkPct)||0;return Math.round((baseAtk()*(1+weaponPowerPct()/100)+eqSum('atk')+skillAtkBonus())*(1+combatMeal/100)); }
...
function equipSkill(k,slot,refresh){ const s=SKILLS[k]; if(!s||s.type!=='active'||!skillUnlocked(k)){log('只有已解锁的主动技能可以装备。','warn');return;} slot=slot==null?((state.skillSlots||[]).findIndex(x=>!x)):slot; if(slot<0)slot=state.skillSlotSel||0;
  const old=equippedSlot(k); if(old>=0)state.skillSlots[old]=null; state.skillSlots[slot]=k; state.skillSlotSel=slot; log('技能栏 '+(slot+1)+' 已装配【'+s.name+'】。','good',{toast:false}); if(refresh)refresh();else render(); }
function unequipSkill(slot,refresh){ if(!state.skillSlots[slot])return; state.skillSlots[slot]=null; state.skillSlotSel=slot; if(refresh)refresh();else render(); }
...
function buyEchoUpgrade(id,onChanged){ const e=ECHO_UPGRADES[id],cost=echoUpgradeCost(id); if(!e||state.meta.echo<cost){log('回响不足。','warn');return;}
  state.meta.echo-=cost; state.meta.echoUp[id]++; refreshEchoMultipliers();state.echoOpen=true;markInlineChange('echo',id);log('✦ 回响强化【'+e.name+'】升至 '+state.meta.echoUp[id]+' 级。','good',{toast:false}); if(typeof onChanged==='function'){onChanged();renderTop();save();}else render(); }
...
/* 调查进度只统计已经确认的固定剧情线索；空搜、战斗和普通物资不冒充任务进度。
   已经在接取前找到的现场证据仍然有效，避免事件耗尽后任务永久卡死。 */
function questSearchCount(q){return Math.max(0,state.areaSearch[q.target]||0);}
...
function questProgress(q){
  if(q.type==='search')return Math.min(q.count,questSearchCount(q))+'/'+q.count+' 条线索';
...
const FIELD_MARKER_SLOT_PREFERENCES={
  resource:[8],npc:[6,7],mission:[5],operation:[9],action:[10],boss:[2],threat:[1],system:[4],route:[0,3,11],
};
const FIELD_FOG_RADII={resource:[19,15],npc:[17,14],mission:[18,14],operation:[18,14],action:[17,14],boss:[18,15],threat:[15,12],system:[18,14],route:[15,12]};
...
function fieldMapMarkerCandidates(id){
  ...
  if(LOCATIONS[id].boss&&!metaFlag(LOCATIONS[id].bossFlag)){const boss=ENEMIES[LOCATIONS[id].boss],need=LOCATIONS[id].bossSearch||1;markers.push({id:'boss:'+LOCATIONS[id].boss,kind:'boss',icon:'combat',label:boss.name,target:LOCATIONS[id].boss,revealed:attempts>=need});}
...
function showSiteSheet(){renderSiteSheet($('panel'));save();}
function openRouteObstacle(from,to){state.siteSheet={kind:'routeObstacle',from,to};showSiteSheet();}
...
function openSiteSheet(kind,id){ state.siteSheet={kind,id}; showSiteSheet(); }
function closeSiteSheet(){ state.siteSheet=null;document.querySelectorAll('.site-sheet-backdrop').forEach(x=>x.remove());save(); }
...
const CLOUD_ENDPOINT='https://abyssal-echoes-ark.netlify.app/api/cloud-save',CLOUD_COOLDOWN_SECONDS={read:10,write:30},TAB_LEASE_MS=10000;
...
const LOCAL_BACKUP_FORMAT='abyss_echo_backup_v2',LOCAL_BACKUP_PAYLOAD_FORMAT='abyss_echo_save_payload_v1',LOCAL_BACKUP_ITERATIONS=210000;
function localBackupPassword(value){const password=String(value||'');if(password.length<8)throw new Error('备份密码至少需要 8 个字符');return password;}
function localBackupCrypto(){const api=globalThis.crypto;if(!api||!api.subtle||typeof api.getRandomValues!=='function')throw new Error('当前系统不支持安全加密，无法导出或导入备份');return api;}
...
function base64ToBytes(value){const text=String(value||'');if(!text||text.length%4||!/^[A-Za-z0-9+/]+={0,2}$/.test(text))throw new Error('备份文件结构无效');const binary=atob(text),bytes=new Uint8Array(binary.length);for(let i=0;i<binary.length;i++)bytes[i]=binary.charCodeAt(i);return bytes;}
function localBackupAad(envelope){return new TextEncoder().encode(JSON.stringify([envelope.format,envelope.exportedAt,envelope.kdf,envelope.iterations,envelope.salt,envelope.cipher,envelope.iv]));}
function validLocalBackupEnvelope(value){
  if(!value||typeof value!=='object'||Array.isArray(value))return false;
  const keys=Object.keys(value).sort().join(',');
  return keys==='cipher,data,exportedAt,format,iterations,iv,kdf,salt'&&value.format===LOCAL_BACKUP_FORMAT&&value.kdf==='PBKDF2-SHA256'&&value.iterations===LOCAL_BACKUP_ITERATIONS&&value.cipher==='AES-256-GCM'&&typeof value.exportedAt==='string'&&!Number.isNaN(Date.parse(value.exportedAt))&&typeof value.salt==='string'&&typeof value.iv==='string'&&typeof value.data==='string';
}
async function deriveLocalBackupKey(password,salt,usage){
  const api=localBackupCrypto(),material=await api.subtle.importKey('raw',new TextEncoder().encode(localBackupPassword(password)),'PBKDF2',false,['deriveKey']);
  return api.subtle.deriveKey({name:'PBKDF2',salt,iterations:LOCAL_BACKUP_ITERATIONS,hash:'SHA-256'},material,{name:'AES-GCM',length:256},false,[usage]);
}
async function createLocalBackup(password){
  const api=localBackupCrypto(),salt=api.getRandomValues(new Uint8Array(16)),iv=api.getRandomValues(new Uint8Array(12));
  const envelope={format:LOCAL_BACKUP_FORMAT,exportedAt:new Date().toISOString(),kdf:'PBKDF2-SHA256',iterations:LOCAL_BACKUP_ITERATIONS,salt:bytesToBase64(salt),cipher:'AES-256-GCM',iv:bytesToBase64(iv),data:''};
  const payload=JSON.stringify({format:LOCAL_BACKUP_PAYLOAD_FORMAT,save:JSON.parse(JSON.stringify(state))}),key=await deriveLocalBackupKey(password,salt,'encrypt');
  const encrypted=await api.subtle.encrypt({name:'AES-GCM',iv,additionalData:localBackupAad(envelope),tagLength:128},key,new TextEncoder().encode(payload));
...
async function parseLocalBackup(text,password){
  ...
  const api=localBackupCrypto(),key=await deriveLocalBackupKey(password,salt,'decrypt');let decrypted;
  try{decrypted=await api.subtle.decrypt({name:'AES-GCM',iv,additionalData:localBackupAad(parsed),tagLength:128},key,data);}catch(_){throw new Error('密码错误，或备份文件已被修改');}
  let payload;try{payload=JSON.parse(new TextDecoder().decode(decrypted));}catch(_){throw new Error('备份解密后内容无效');}
  if(!payload||payload.format!==LOCAL_BACKUP_PAYLOAD_FORMAT||!validGameSave(payload.save))throw new Error('备份解密后内容无效');return payload.save;
}
...
function queueNpcFirstContact(name,location,text){
  ...
  return queueStoryScene({npc:name,location,onceKey:'npc-'+name,kind:'contact',eyebrow:'FIRST CONTACT // '+((LOCATIONS[location]&&LOCATIONS[location].zone)||'UNKNOWN'),title:'发现幸存者 · '+name,text:text||NPC_FIRST_CONTACT[name],action:'走近交谈',onComplete:()=>approachDiscoveredNpc(name,location)});
}
function approachDiscoveredNpc(name,location){if(P().location===location&&npcLocation(name)===location&&fieldNpcMapped(name,location)&&state.screen==='play'&&!state.combat)openNpcPanel(name);}
function queueFieldNpcDiscovery(name,location,entry){
  ...
  return queueStoryScene({npc:name,location,onceKey:'field-npc-'+name+'-'+location,kind:'contact',eyebrow:(entry.relocated?'CONTACT REACQUIRED':'SURVIVOR SIGNAL')+' // '+((LOCATIONS[location]&&LOCATIONS[location].zone)||'UNKNOWN'),title:entry.title||((entry.relocated?'重新汇合 · ':'信号确认 · ')+name),lines,action:entry.action||'建立联系',onComplete:()=>approachDiscoveredNpc(name,location)});
}
...
function installInteractionFeedback(){
  ...
  let lastPress=null;
  const release=e=>{const press=pressedPointers.get(e.pointerId);pressedPointers.delete(e.pointerId);if(!press)return;if(e.type==='pointercancel')press.moved=true;lastPress={...press,until:Date.now()+450};press.button.classList.remove('is-touching');};
  document.addEventListener('pointerdown',e=>{unlockAudio();lastPress=null;const b=buttonFrom(e);if(!b||b.disabled)return;const previous=pressedPointers.get(e.pointerId);if(previous)previous.button.classList.remove('is-touching');pressedPointers.set(e.pointerId,{button:b,x:e.clientX,y:e.clientY,moved:false,touch:e.pointerType==='touch'});b.classList.add('is-touching');},{passive:true});
  document.addEventListener('pointermove',e=>{const press=pressedPointers.get(e.pointerId);if(press&&Math.hypot(e.clientX-press.x,e.clientY-press.y)>10){press.moved=true;press.button.classList.remove('is-touching');}},{passive:true});
  ...
  document.addEventListener('click',e=>{const b=buttonFrom(e);if(!b||b.disabled)return;const recent=lastPress&&lastPress.button===b&&lastPress.until>Date.now();if(e.detail!==0&&recent&&lastPress.moved){e.preventDefault();e.stopImmediatePropagation();return;}unlockAudio().then(ok=>{if(ok)playSfx('tap');});if(state&&state.vibration!==false&&(e.pointerType==='touch'||(recent&&lastPress.touch))&&navigator.vibrate)try{navigator.vibrate(8);}catch(_){}if(feedbackBatch){clearTimeout(feedbackFlushTimer);feedbackBatch=null;}const batch={entries:[]};feedbackBatch=batch;feedbackFlushTimer=setTimeout(()=>{if(feedbackBatch===batch)feedbackBatch=null;flushFeedbackBatch(batch);},0);},true);
...
let activeThumbSheet=null;
function mountThumbSheet(content,options={}){
  if(activeThumbSheet)activeThumbSheet.close(false);
  const app=$('app'),source=document.activeElement,root=el('div','thumb-sheet-backdrop'),panel=el('section','thumb-sheet ui-dialog '+(options.className||'')),grip=el('button','thumb-sheet-grip','<span></span>'),body=el('div','thumb-sheet-body'),footer=el('footer','thumb-sheet-footer');
  const siblings=Array.from(app.children).map(node=>({node,inert:node.inert}));
  panel.setAttribute('role','dialog');panel.setAttribute('aria-modal','true');panel.setAttribute('aria-label',options.label||'详情');panel.tabIndex=-1;
  grip.type='button';grip.setAttribute('aria-label','收起详情');
  if(content instanceof Node)body.appendChild(content);else if(typeof content==='string')body.innerHTML=content;
  let closed=false,start=null;
  const close=(restoreFocus=true)=>{
    if(closed)return;closed=true;root.remove();siblings.forEach(({node,inert})=>{node.inert=inert;});app.classList.remove('thumb-sheet-open');document.removeEventListener('keydown',onKey);
    if(activeThumbSheet===api)activeThumbSheet=null;
    if(restoreFocus&&source&&source.isConnected)source.focus({preventScroll:true});
    if(options.onClose)options.onClose();
  };
  const onKey=e=>{
    if(e.key==='Escape'){e.preventDefault();e.stopPropagation();close();return;}
    if(e.key!=='Tab')return;
    const focusable=Array.from(panel.querySelectorAll('button:not(:disabled),input:not(:disabled),select:not(:disabled),summary,[tabindex="0"]')).filter(node=>node.getClientRects().length),first=focusable[0],last=focusable.at(-1);
    if(!first){e.preventDefault();panel.focus();}else if(e.shiftKey&&(document.activeElement===first||document.activeElement===panel)){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}
  };
  root.addEventListener('pointerdown',e=>{start=e.target===root?{x:e.clientX,y:e.clientY}:null;});
  root.addEventListener('pointerup',e=>{if(start&&e.target===root&&Math.hypot(e.clientX-start.x,e.clientY-start.y)<10)close();start=null;});
  grip.onclick=()=>close();panel.append(grip,body,footer);root.appendChild(panel);
  const api={root,panel,body,footer,close};activeThumbSheet=api;siblings.forEach(({node})=>{node.inert=true;});app.classList.add('thumb-sheet-open');app.appendChild(root);document.addEventListener('keydown',onKey);panel.focus({preventScroll:true});return api;
}
function handleGameBack(){
  if(storySceneActive||document.querySelector('.story-cutscene'))return true;
  if(activeThumbSheet){activeThumbSheet.close();return true;}
  if(document.body.classList.contains('launch-pending'))return false;
  if($('save-transfer-modal')){closeSaveTransfer();return true;}
  if(state.siteSheet){closeSiteSheet();return true;}
  if(state.combat||state.screen!=='play'||tutorialActive())return true;
  if(state.mapOpen){closeContextMap();return true;}
  if(state.npcTarget){state.npcTarget=null;if(P().location==='camp')state.campView='home';renderPanelTop();return true;}
  const exit=$('panel')?.querySelector('.field-map-drawer-close,.ui-workspace__exit,.map-close,.gene-back,.tx-x[aria-label="关闭科技台"]');
  if(exit&&!exit.disabled){exit.click();return true;}
  if(state.tab==='set'&&settingsView!=='main'){settingsView='main';render();return true;}
  if(state.tab!=='act'){state.tab='act';render();return true;}
  if(state.campBuilding||state.campView!=='home'){state.campBuilding=null;state.campView='home';render();return true;}
  return false;
}
globalThis.abyssHandleBack=handleGameBack;
function uiIcon(name,cls){ return '<svg class="ui-icon'+(cls?' '+cls:'')+'" aria-hidden="true" focusable="false"><use href="#icon-'+name+'"></use></svg>'; }
function uiModuleHeader(icon,code,name,desc,meta,cls){
  const header=el('header',(cls?cls+' ':'')+'ui-module-header');
  header.innerHTML='<span class="ui-module-header__mark">'+uiIcon(icon)+'</span><span><small>'+code+'</small><h1>'+name+'</h1><p>'+desc+'</p></span>'+(meta?'<em>'+meta+'</em>':'');
  return header;
}
function uiSectionHeader(code,name,meta,cls){
  return el('div',(cls?cls+' ':'')+'ui-section-header','<span><small>'+code+'</small><b>'+name+'</b></span>'+(meta?'<em>'+meta+'</em>':''));
}
function workspaceExit(label,fn,options){
  options=options||{};
...
const BUILDING_ART_FILE={research:'research.png',garden:'garden-v2.png'};
function buildingUiIcon(id){const file=BUILDING_ART_FILE[id]||(id+'.webp');return '<img class="building-art" data-building="'+id+'" src="assets/building-art-v1/'+file+'?v=2" alt="" draggable="false">';}
...
function renderGaugeValue(id,label,current,limit){
  const root=$(id),currentNode=root.querySelector('.g-current'),limitNode=root.querySelector('.g-limit');
  const currentText=String(current),limitText=String(limit);
  if(currentNode&&limitNode){currentNode.textContent=currentText;limitNode.textContent=limitText;}
  else root.textContent=currentText+'/'+limitText;
  root.classList.toggle('is-long',currentText.length+limitText.length>11);
  root.setAttribute('aria-label',label+' '+currentText+' / '+limitText);
}
function renderTop(){
  const maxh=maxHp(), maxs=Math.round(maxStamina());
  P().hp=Math.min(P().hp,maxh); P().stamina=Math.min(P().stamina,maxs);
  renderGaugeValue('hp','生命',Math.max(0,P().hp),maxh); $('hp-fill').style.width=Math.max(0,P().hp/maxh*100)+'%';
  renderGaugeValue('stamina','能量',P().stamina,maxs); $('st-fill').style.width=Math.max(0,P().stamina/maxs*100)+'%';
  $('time').textContent=fmtTime();
  $('pt-label').textContent='轮回'+state.meta.playthrough;
}
function primaryTabKey(){if(state.tab==='char'&&state.charView==='careers')return 'career';if(state.tab==='char'&&state.charView==='skills')return 'skill';return state.tab;}
function renderTabbar(){const active=primaryTabKey();document.querySelectorAll('#tabbar .tab').forEach(b=>b.classList.toggle('active',b.dataset.tab===active));const sb=$('set-btn');if(sb)sb.classList.toggle('on',state.tab==='set');}
...
function render(){ if(_npcCapturing)return; if(activeThumbSheet)activeThumbSheet.close(false);normalizePanelNavigationState();renderTop(); const box=$('panel'),activeView=panelView(),keepFieldViewport=fieldViewportCanStayMounted(box,activeView);if(keepFieldViewport)Array.from(box.children).forEach(node=>{if(node!==retainedFieldViewport)node.remove();});else box.innerHTML='';box.classList.remove('camp-home','tutorial-panel','recipe-station-page','facility-operation-page','watch-defense-page','skill-console-page','skills-page','thumb-skills-page','careers-page','character-page','npc-screen','settlement-shop-page','field-console','expedition-board','settlement-console','map-mode','camp-map-page','settings-home','settings-cloud','tasks-console','ui-page','ui-workspace','ui-canvas-workspace','core-control-page','ending-page');
  ...
  $('app').classList.toggle('facility-fullpage',activeView==='facility');
  $('app').classList.toggle('npc-fullpage',activeView==='npc');
  $('app').classList.toggle('workspace-fullpage',immersiveWorkspace||canvasWorkspace);
...
function renderSiteSheet(box){
  ...
  if(ref.kind==='operation'){
  ...
  }else if(ref.kind==='masteryBook'){
    if(!has('masteryManual')){state.siteSheet=null;return;}const held=state.inv.masteryManual||0,ids=Object.keys(MASTERIES);let selected=MASTERIES[ref.mastery]?ref.mastery:ids[0],count=Math.min(held,Math.max(1,Math.floor(Number(ref.qty)||1))),input,confirm;const choiceRefs=[],choiceMarkup=(k,m)=>'<span><small>LV '+masteryLv(k)+'</small><b>'+m.name+'</b></span><em>'+masteryEffectText(k,masteryLv(k))+'</em>',syncPreview=()=>{const next=masteryLv(selected)+count;confirm.innerHTML='<span><small>确认研读 '+count+' 本</small><b>'+MASTERIES[selected].name+' · Lv'+masteryLv(selected)+' → Lv'+next+'</b><em>'+masteryEffectText(selected,next)+'</em></span>';},setCount=value=>{count=Math.min(held,Math.max(1,Math.floor(Number(value)||1)));ref.qty=count;input.value=String(count);syncPreview();},selectMastery=k=>{selected=k;ref.mastery=k;choiceRefs.forEach(row=>{const active=row.k===k;row.button.classList.toggle('selected',active);row.button.setAttribute('aria-pressed',active?'true':'false');});syncPreview();};ref.qty=count;ref.mastery=selected;sheet.classList.add('mastery-book-sheet');sheet.innerHTML='<div class="site-sheet-grip"></div><div class="site-sheet-head"><span>'+itemUiIcon('masteryManual')+'</span><div><small>MASTERY MANUAL · 持有 '+held+'</small><b>定向研读精通</b></div></div><p>先选择要提升的精通，再设定使用数量；100 级后每一级都会继续强化专属超限效果。</p>';
    const choices=el('div','mastery-book-grid');Object.entries(MASTERIES).forEach(([k,m])=>{const button=el('button','mastery-book-choice ui-card'+(k===selected?' selected':''),choiceMarkup(k,m));button.setAttribute('aria-pressed',k===selected?'true':'false');button.onclick=()=>selectMastery(k);choiceRefs.push({button,k});choices.appendChild(button);});sheet.appendChild(choices);
    const batch=el('div','mastery-book-batch ui-card','<span><small>USE QUANTITY</small><b>本次使用数量</b></span>'),controls=el('div','mastery-book-controls');input=el('input','mastery-book-quantity ui-input');[-10,-1].forEach(delta=>{const button=el('button','ui-button',String(delta));button.onclick=()=>setCount(count+delta);controls.appendChild(button);});input.type='number';input.min='1';input.max=String(held);input.value=String(count);input.oninput=()=>setCount(input.value);input.onblur=()=>setCount(input.value);controls.appendChild(input);[1,10,100].forEach(delta=>{const button=el('button','ui-button','+'+delta);button.onclick=()=>setCount(count+delta);controls.appendChild(button);});batch.appendChild(controls);sheet.appendChild(batch);
    confirm=el('button','site-sheet-primary mastery-book-confirm');confirm.onclick=()=>useMasteryManual(selected,count,refreshBagPanel);syncPreview();sheet.appendChild(close);sheet.appendChild(confirm);
  }else if(ref.kind==='beaconReward'){
    const reward=ref.reward;if(!reward){state.siteSheet=null;return;}sheet.classList.add('beacon-reward-sheet');sheet.innerHTML='<div class="site-sheet-grip"></div><div class="site-sheet-head"><span>'+uiIcon('check')+'</span><div><small>BEACON FLOOR CLEAR</small><b>信标第 '+reward.floor+' 层奖励</b></div></div><div class="beacon-reward-grid">'+reward.items.map(row=>'<div><span>'+itemUiIcon(row.id)+'</span><b>'+ITEMS[row.id].name+'</b><em>×'+row.amount+'</em></div>').join('')+'</div><div class="site-sheet-status ready">'+(reward.unlocked?'已解锁第 '+reward.nextFloor+' 层':'重复通关奖励已结算')+' · 奖励面板会保留到你主动关闭</div>';
    sheet.appendChild(close);const action=el('button','site-sheet-primary','收下全部奖励');action.onclick=closeSiteSheet;sheet.appendChild(action);
  }else if(ref.kind==='item'){
    const it=ITEMS[ref.id];if(!it||!has(ref.id)||state.tab!=='bag'){state.siteSheet=null;return;}
    const typeName=({equip:'装备',use:'消耗品',book:'技能书',masteryBook:'精通手册',key:'关键道具',trophy:'结局道具'})[it.type]||'物品',slot=SLOTS.find(x=>x[0]===it.slot),stats=it.type==='equip'?statTags(it):'',grade=it.grade&&EQUIPMENT_GRADES[it.grade],desc=it.desc||(it.type==='equip'?'装备至【'+slot[1]+'】后生效。'+(grade?' '+grade.name+'以'+grade.material+'为核心材料；'+grade.rule+'。':''):it.type==='book'?'研读后提升【'+SKILLS[it.skill].name+'】熟练度。':it.type==='use'?'可从背包主动使用。':it.type==='key'?'探索与任务中自动验证，无需主动使用。':'远征过程中取得的特殊物品。');
    ...
    sheet.appendChild(close);const usable=it.type==='equip'||it.type==='use'||it.type==='book'||it.type==='masteryBook',label=it.type==='equip'?'装备到'+slot[1]:it.type==='masteryBook'?'选择精通':it.type==='book'?'研读'+it.name:it.type==='use'?'使用'+it.name:'收起详情',action=el('button','site-sheet-primary',label);
    ...
    action.onclick=usable?()=>{if(it.type==='masteryBook'){state.siteSheet={kind:'masteryBook'};showSiteSheet();}else{state.siteSheet=null;if(it.type==='equip'){state.bagSel=null;equip(it.slot,ref.id,refreshBagPanel);}else useItem(ref.id,refreshBagPanel);}}:closeSiteSheet;sheet.appendChild(action);
...
function refreshFacilityOperation(id){
  id=id||state.campBuilding;const panel=$('panel'),current=panel&&panel.querySelector&&panel.querySelector('.facility-operation-screen');
  if(!panel||!current||id!==state.campBuilding){render();return;}
  const positions=['.facility-operation-content','.recipe-station-top','.station-detail-body'].map(selector=>{const node=current.querySelector(selector);return {selector,top:node?node.scrollTop:0};});
  renderBuilding(panel,id);const screens=panel.querySelectorAll('.facility-operation-screen'),fresh=screens[screens.length-1];if(!fresh||fresh===current){render();return;}
  replaceMountedNode(current,fresh,current.parentNode);positions.forEach(position=>{const node=fresh.querySelector(position.selector);if(node)node.scrollTop=position.top;});renderTop();renderSiteSheet(panel);renderTabbar();save();
}
...
function settlementExplorationCount(){
  return Object.keys(LOCATIONS).filter(id=>id!=='camp'&&regionForLocation(id)!=='settlement'&&(state.discovered&&state.discovered[id]||state.visited&&state.visited[id]||state.meta&&state.meta.spaceDiscovered&&state.meta.spaceDiscovered[id])).length;
}
function settlementShopProgress(){
  const explored=settlementExplorationCount(),tier=[...SETTLEMENT_SHOP_TIERS].reverse().find(entry=>explored>=entry.min)||SETTLEMENT_SHOP_TIERS[0],next=SETTLEMENT_SHOP_TIERS.find(entry=>entry.min>explored)||null;
  const span=next?Math.max(1,next.min-tier.min):1,within=next?Math.max(0,explored-tier.min):span;
  return {explored,tier:tier.id,name:tier.name,code:tier.code,next,nextAt:next&&next.min,remaining:next?next.min-explored:0,percent:next?Math.min(100,Math.round(within/span*100)):100};
}
function settlementShopCatalog(category){
  const tier=settlementShopProgress().tier,filter=SETTLEMENT_SHOP_CATEGORIES[category]?category:'all';
  return Object.entries(SETTLEMENT_SHOP).filter(([,row])=>row.tier<=tier&&(filter==='all'||row.category===filter));
}
function settlementShopUnlocked(itemId){const row=SETTLEMENT_SHOP[itemId];return !!row&&row.tier<=settlementShopProgress().tier;}
function settlementBuyPrice(row,discount){return Math.max(1,Math.ceil(row.buy.crystal*(discount==null?settlementDiscount():discount)));}
function settlementSellTerms(row,discount){
  if(!row||!row.sell)return 0;
  const multiplier=discount==null?settlementDiscount():discount,buyPrice=settlementBuyPrice(row,multiplier);
  if(multiplier<=.8)return {amount:row.buy.amount,crystal:buyPrice};
  const safeMax=Math.max(0,Math.floor(buyPrice*row.sell.amount/row.buy.amount));
  return {amount:row.sell.amount,crystal:Math.min(row.sell.crystal,safeMax)};
}
function settlementSellReward(row,discount){const terms=settlementSellTerms(row,discount);return terms&&terms.crystal||0;}
function settlementTradeQuote(itemId,mode,quantity){
  const row=SETTLEMENT_SHOP[itemId];if(!row)return null;
  const batches=row.unique?1:batchQuantity(quantity);
  if(!settlementShopUnlocked(itemId))return {row,batches,enough:false,reason:'继续探索并提升商路等级后才会供货'};
  if(mode==='buy'){
    const unitCost=settlementBuyPrice(row),unitGain=row.buy.amount,cost=unitCost*batches,gain=unitGain*batches,have=state.inv.crystal||0,owned=row.unique&&ownsItem(itemId);
    return {row,batches,buying:true,costId:'crystal',gainId:itemId,unitCost,unitGain,cost,gain,have,limit:owned?0:Math.floor(have/unitCost),enough:!owned&&have>=cost,reason:owned?'限购装备已经持有':have<cost?'晶体不足':''};
  }
  if(mode==='sell'&&row.sell){
    const terms=settlementSellTerms(row),unitCost=terms.amount,unitGain=terms.crystal,cost=unitCost*batches,gain=unitGain*batches,have=state.inv[itemId]||0;
    return {row,batches,buying:false,costId:itemId,gainId:'crystal',unitCost,unitGain,cost,gain,have,limit:Math.floor(have/unitCost),enough:unitGain>0&&have>=cost,reason:have<cost?(ownsItem(itemId)?'先卸下装备才能出售':'持有数量不足'):unitGain<=0?'该物品不接受回收':''};
  }
  return {row,batches,enough:false,reason:'当前货架不支持这项交易'};
}
function settlementTrade(itemId,mode,quantity,refresh){
...
function renderNpcPanelLegacy(box){
  ...
  if(teachList.length){
    ...
    const tTalk=el('button','npc-tab'+(state.npcTab==='talk'?' active':''),'交谈');tTalk.onclick=()=>{if(state.npcTab==='talk')return;state.npcTab='talk';renderPanelTop();};tabs.appendChild(tTalk);
    const tTeach=el('button','npc-tab'+(state.npcTab==='teach'?' active':''),'精通教学');tTeach.onclick=()=>{if(state.npcTab==='teach')return;state.npcTab='teach';renderPanelTop();};tabs.appendChild(tTeach);
  ...
  if(state.npcTab==='teach'&&teachList.length){
    ...
    teachList.forEach(k=>{
      ...
      const current=masteryEffectText(k,lv),next=masteryEffectText(k,lv+1);
...
function renderNpcMasteries(box,npcName){
  ...
  (NPC_TEACH[npcName]||[]).forEach(k=>{const card=el('div','mastery-card');renderMasteryCard(card,k);masteryGrid.appendChild(card);});box.appendChild(masteryGrid);
}
function renderMasteryCard(card,k){
  const m=MASTERIES[k],lv=masteryLv(k),cost=masteryCost(k),current=masteryEffectText(k,lv),next=masteryEffectText(k,lv+1),can=Object.entries(cost).every(([mat,n])=>(state.inv[mat]||0)>=n);card.className='mastery-card'+inlineChangeClass('mastery',k);
  ...
  const button=el('button',can?'primary':'',lv===0?'学习至 Lv1':'升级至 Lv'+(lv+1));button.disabled=!can;button.onclick=()=>upgradeMastery(k,()=>renderMasteryCard(card,k));card.appendChild(button);
}
...
function renderNpcPanel(box){
  ...
  const terminal=el('section','npc-terminal ui-workspace__portrait '+profile.tone),stage=el('div','npc-stage'),at=npcLocation(npcName),locationName=at&&LOCATIONS[at]?LOCATIONS[at].name:'行踪未知';
  ...
  const actions=el('nav','npc-actions ui-segmented ui-segmented--portrait count-'+modes.length);actions.setAttribute('aria-label','与'+npcName+'互动');
  modes.forEach(mode=>{const button=el('button','npc-action-button'+(state.npcTab===mode.id?' active':''),uiIcon(mode.icon)+'<span>'+mode.label+'</span>');button.dataset.mode=mode.id;button.setAttribute('aria-pressed',state.npcTab===mode.id?'true':'false');button.onclick=()=>{if(state.npcTab===mode.id)return;state.npcTab=mode.id;refreshNpcMode();};actions.appendChild(button);});console.appendChild(actions);
...
function renderConstruction(box){ state.campBuilding=null; state.campView='construct'; const shell=el('div','construction-workspace ui-workspace__shell'),content=el('div','construction-workspace-scroll ui-workspace__scroll');
  const available=CAMP_BUILDINGS.filter(b=>!state.meta.built[b.id]&&hasBuildingTech(b.id));
  content.appendChild(uiModuleHeader('construct','CONSTRUCTION // CONTROL','建筑管理','只显示科技已解锁、当前可以落地的设施',String(available.length).padStart(2,'0')+' BLUEPRINTS','construction-head'));
  const summary=el('div','build-summary','<span><b>'+available.length+'</b><small>可建造设施</small></span><p>新建筑完成后会出现在营地主页；升级在各建筑内部进行。</p>');content.appendChild(summary);
  if(!available.length)content.appendChild(el('div','build-empty','<span>'+uiIcon('construct')+'</span><b>当前没有新的建筑蓝图</b><p>继续探索并在科技树研究建筑类节点，解锁后会自动出现在这里。</p>'));
  else{const list=el('div','build-list');available.forEach(b=>{const can=canAfford(b.cost),card=el('article','build-card ui-card '+b.tone);card.innerHTML='<div class="build-card-main"><span class="bc-icon ui-art-frame">'+buildingUiIcon(b.id)+'</span><span><small>NEW FACILITY</small><b>'+b.name+'</b><em>'+b.desc+'</em></span></div><div class="build-cost">'+costText(b.cost)+'</div>';const action=el('button',can?'primary':'','建造 '+b.name);action.disabled=!can;action.onclick=()=>buildFacility(b);card.appendChild(action);list.appendChild(card);});content.appendChild(list);}
  shell.appendChild(content);shell.appendChild(workspaceExit('关闭建筑管理',()=>{state.campView='home';renderPanelTop();},{ariaLabel:'关闭建筑管理并返回营地'}));box.appendChild(shell);
}
...
function renderSpaceRoutes(box,all){
  ...
  const list=el('div','operation-list');routes.forEach(route=>{const status=spaceFlightStatus(route),dir=status.dir,dest=dir&&dir.dest,loc=dest==='camp'?LOCATIONS.camp:LOCATIONS[dest],label=dir?((dir.forward?'前往 ':'返航 ')+(loc?loc.name:dest)):'航线未接入当前港口';list.appendChild(operationRow(uiIcon('expedition'),route.name,label,status.text,'启航',!status.ok,()=>launchSpaceRoute(route.id),status.ok?'primary':''));});sec.appendChild(list);
...
function renderOutpostPanel(box){
  ...
  if(op.status==='operational'){const actions=el('div','operation-list');actions.appendChild(operationRow(uiIcon('camp'),'前哨维护','恢复全部状态并建立检查点','用时 8 小时','休整',false,restAtOutpost,'primary'));if(op.parts.exoExtractor)actions.appendChild(operationRow(uiIcon('salvage'),'收取异星资源','每日自动采集一次',facilityUsedToday('outpost:'+rid)?'今日已收取':'可以收取','收取',facilityUsedToday('outpost:'+rid),harvestOutpost,''));sec.appendChild(actions);}
  box.appendChild(sec);
}
...
function renderSettlementShop(box,expanded){
  const progress=settlementShopProgress(),available=settlementShopCatalog('all');
  if(!expanded){
    const sec=el('section','settlement-services');sec.appendChild(el('div','camp-section-head','<span><small>EXPEDITION MARKET</small><b>中枢商店</b></span><em>商路等级 0'+progress.tier+' · '+available.length+' 种商品</em>'));
    const open=el('button','camp-command-card market-entry','<span class="cc-icon">'+itemUiIcon('crystal')+'</span><span class="cc-copy"><small>MARKET TERMINAL // ROUTE 0'+progress.tier+'</small><b>打开交易操作台</b><em>物资、装备与特殊货品随地图探索持续扩充</em></span><span class="command-access"><small>OPEN</small><i>'+uiIcon('chevron-right')+'</i></span>');open.onclick=()=>{state.settlementShopOpen=true;renderPanelTop();};sec.appendChild(open);box.appendChild(sec);return;
  }
...
function renderFieldMarkerDrawer(drawer,marker,id){
  const markerKey=id+'|'+marker.id;if(drawer._fieldMarkerKey===markerKey&&drawer._refreshFieldMarker){drawer._refreshFieldMarker();return;}drawer._fieldMarkerKey=markerKey;drawer._refreshFieldMarker=null;
  ...
  close.type='button';close.setAttribute('aria-label','关闭地图地点详情');close.onclick=()=>{fieldMarkerSelection=null;drawer._fieldMarkerKey=null;drawer._refreshFieldMarker=null;drawer.className='field-map-drawer';drawer.innerHTML='';const selected=drawer.parentNode&&drawer.parentNode.querySelector('.field-map-marker.is-selected');if(selected)selected.classList.remove('is-selected');};head.appendChild(close);drawer.appendChild(head);
  const body=el('div','field-map-drawer-body');drawer.appendChild(body);
  const action=(label,meta,disabled,fn,cls)=>{const button=el('button','field-map-drawer-action '+(cls||''),'<span><b>'+label+'</b><small>'+meta+'</small></span>'+uiIcon('chevron-right'));button.type='button';button.disabled=!!disabled;button.onclick=fn;body.appendChild(button);return button;};
  if(marker.kind==='resource'){
    const site=resourceSiteOf(id);
    body.appendChild(el('p','',site?'已标定产出：'+site.yield.map(key=>ITEMS[key].name).join(' · '):'资源读数缺失'));
    const gather=action('开始'+resourceActionVerb(site,LOCATIONS[id].profile),'',false,()=>explore('gather'),'resource'),readout=gather.querySelector('small');
    drawer._refreshFieldMarker=()=>{const work=resourceWorkStatus(id),remaining=gatherAvailable(id),skill=activeFieldGatherSkill(id),status=skill?fieldGatherSkillStatus(skill,id):null;gather.disabled=!remaining||!work.ok||!!status&&!status.ok;readout.textContent=remaining?(status?status.text:'储量 '+remaining+'/'+gatherLimit(id)+' · '+work.text):'储量耗尽 · 约 '+resourceRecoveryRemaining(id)+' 小时恢复';};drawer._refreshFieldMarker();
    ...
    const siteAction=LOCATION_ACTIONS[id];body.appendChild(el('p','',siteAction.desc));const operate=action(siteAction.name,'',false,()=>performLocationAction(id),'action'),readout=operate.querySelector('small');drawer._refreshFieldMarker=()=>{const remaining=locationActionRemaining(id),status=locationActionStatus(id);operate.disabled=!remaining||!status.ok;readout.textContent=remaining?status.text:'今日次数已用完';};drawer._refreshFieldMarker();
...
function refreshFieldAfterAction(id){
  const panel=$('panel');if(P().location!==id||panelView()!=='explore'||state.siteSheet||!fieldViewportCanStayMounted(panel,'explore')){render();return;}
  const markers=fieldMapMarkers(id),fog=fieldFogState(id,markers),attempts=exploreAttempts(id),copy=panel.querySelector('.field-map-head-copy em'),status=retainedFieldViewport.querySelector('.field-map-empty,.field-map-readout'),fresh=fieldMapStatusNode(id,attempts,markers),drawer=retainedFieldViewport.querySelector('.field-map-drawer'),button=panel.querySelector('.field-explore-button');
  if(copy)copy.textContent='已发现 '+fog.revealed+'/'+fog.total+' · 勘察 '+attempts+' 次';
  if(status){status.className=fresh.className;status.innerHTML=fresh.innerHTML;}
  if(drawer&&drawer._refreshFieldMarker)drawer._refreshFieldMarker();
  if(button){const step=button.querySelector('small'),label=button.querySelector('b'),hint=button.querySelector('em'),cost=button.querySelector('strong');if(step)step.textContent='PRIMARY SURVEY // '+String(attempts+1).padStart(2,'0');if(label)label.textContent=attempts?'继续探索':'开始探索';if(hint)hint.textContent=attempts?'扩大测绘范围并寻找新地点':'从当前落脚点建立第一段地图';const costText=cost&&Array.from(cost.childNodes).find(node=>node.nodeType===3);if(costText)costText.textContent='体力 -'+areaActionCost(1);}
  renderTop();save();flushStoryScenes();
}
...
function renderFieldExpedition(box,id){
  ...
  head.appendChild(mark);head.appendChild(copy);
...
function skillEffectClass(ref){if(ref.startsWith('mastery:'))return MASTERIES[ref.slice(8)]&&MASTERIES[ref.slice(8)].route==='combat'?'rhythm':'grid';return SKILL_EFFECT_UI[ref]||'lance';}
function skillToneClass(ref){
  if(ref.startsWith('mastery:'))return MASTERIES[ref.slice(8)]&&MASTERIES[ref.slice(8)].route==='combat'?'tone-power':'tone-mastery';
...
const CAREER_UI={
  vanguard:{icon:'career-vanguard',code:'VANGUARD',role:'持续火力'},noviceScout:{icon:'career-vanguard',code:'SCOUT',role:'战场侦察'},
  bulwark:{icon:'career-bulwark',code:'BULWARK',role:'阵地防卫'},noviceGuard:{icon:'career-bulwark',code:'GUARD',role:'基础防卫'},
  infiltrator:{icon:'career-infiltrator',code:'PHASE',role:'规则穿透'},noviceStriker:{icon:'career-infiltrator',code:'STRIKER',role:'近战打击'},
  salvager:{icon:'career-salvager',code:'SALVAGER',role:'采掘回收'},noviceCollector:{icon:'career-salvager',code:'SCAVENGER',role:'野外采集'},
  fabricator:{icon:'career-fabricator',code:'FABRICATOR',role:'精密制造'},noviceApprentice:{icon:'career-fabricator',code:'APPRENTICE',role:'工程维护'},
  biologist:{icon:'career-biologist',code:'BIOLOGIST',role:'生态培育'},noviceGrower:{icon:'career-biologist',code:'CULTIVATOR',role:'菌圃维护'},
};
function careerUi(id){return CAREER_UI[id]||{icon:'personnel',code:'UNASSIGNED',role:'未定方向'};}
const MAIN_CAREER_VISUAL={vanguard:'vanguard',noviceScout:'vanguard',bulwark:'bulwark',noviceGuard:'bulwark',infiltrator:'infiltrator',noviceStriker:'infiltrator'};
function careerPortraitSrc(id,appearance){const role=MAIN_CAREER_VISUAL[id],sex=appearance==='female'?'female':'male';return role?'assets/career-portraits-v1/'+role+'-'+sex+'.png?v=1':'assets/loadout-'+sex+'-v2.png?v=1';}
function activeCareerPortraitSrc(){const r=careerRecord('main');return careerPortraitSrc(r&&r.id,state.playerAppearance);}
function careerPortraitMarkup(id,cls){return '<img class="'+(cls||'career-role-art')+'" src="'+careerPortraitSrc(id,state.playerAppearance)+'" alt="" draggable="false">';}
...
function renderCharPanel(box){
  ...
  const statMarkup=list=>list.map(c=>'<span class="ui-stat-chip"><small>'+c[0]+'</small><b>'+c[1]+'</b></span>').join('');
  const main=careerRecord('main'),profile=el('section','char-rpg-stage');
  profile.innerHTML='<div class="doll-art-host char-rpg-portrait">'+dollArt()+'</div><header class="char-rpg-identity"><small>方舟幸存者</small><h1>'+(main?careerDefinition(main.id).name:'无名旅人')+'</h1><span>Lv '+P().level+' · '+(state.playerAppearance==='female'?'女性':'男性')+'</span></header><div class="char-rpg-readout"><div class="char-xp"><span>经验 '+P().xp+' / '+xpNeed(P().level)+'</span><div class="xpbar"><div class="xpfill" style="width:'+Math.min(100,P().xp/xpNeed(P().level)*100)+'%"></div></div></div><div class="char-profile-stats ui-stat-grid">'+statMarkup(cells().slice(0,6))+'</div></div>';
  box.appendChild(profile);
  const artHost=profile.querySelector('.char-rpg-portrait');let artSignature;
  const syncPortrait=()=>{const next=dollAppearanceSignature();if(!artHost||artSignature===next)return;artSignature=next;return refreshDollArt(artHost);};
  const syncStats=()=>{const readout=profile.querySelector('.char-profile-stats');if(readout)readout.innerHTML=statMarkup(cells().slice(0,6));const current=careerRecord('main'),name=profile.querySelector('.char-rpg-identity h1'),identity=profile.querySelector('.char-rpg-identity>span');if(name)name.textContent=current?careerDefinition(current.id).name:'无名旅人';if(identity)identity.textContent='Lv '+P().level+' · '+(state.playerAppearance==='female'?'女性':'男性');syncPortrait();};
  ...
  const echo=el('button','echo-entry',uiIcon('core')+'<b>回响强化</b><small>回响 '+state.meta.echo+'</small>');echo.onclick=()=>{
    ...
    Object.keys(ECHO_UPGRADES).forEach(id=>{const button=el('button','char-echo-upgrade');button.dataset.echo=id;button.onclick=()=>{buyEchoUpgrade(id,sync);button.classList.remove('just-updated');void button.offsetWidth;button.classList.add('just-updated');};refs.push({id,button});list.appendChild(button);});sync();
    const sheet=mountThumbSheet(body,{className:'char-thumb-sheet',label:'回响强化'}),back=el('button','char-sheet-back','返回角色');back.onclick=()=>sheet.close();sheet.footer.appendChild(back);
  };dock.appendChild(echo);
  const advanced=el('button','stats-entry',uiIcon('combat')+'<b>战斗属性</b><small>距离与加成</small>');advanced.onclick=()=>{const body=el('section','char-stats-body','<header class="char-sheet-heading"><small>当前装配的实际属性</small><h2>战斗属性</h2></header><div class="char-advanced-stats ui-stat-grid">'+statMarkup(cells())+'</div>'),sheet=mountThumbSheet(body,{className:'char-thumb-sheet',label:'详细战斗属性'}),back=el('button','char-sheet-back','返回角色');back.onclick=()=>sheet.close();sheet.footer.appendChild(back);};dock.appendChild(advanced);box.appendChild(dock);
}
...
const EQUIP_ICON={
  ...
  boots:'boots',magboots:'energy',miningHarness:'exorig',workGloves:'gloves',servoGauntlet:'power-armor',nanoWeaveGloves:'medical',phaseGrip:'phase',critCore:'sensor',lsChip:'lifesteal',dodgeMod:'phase',penMod:'pierce',neuralFilter:'implant',capacitorPack:'backpack',
...
const EQUIPMENT_PORTRAIT_TONE={
  ...
  boots:'kinetic',magboots:'energy',miningHarness:'salvage',critCore:'danger',lsChip:'bio',dodgeMod:'phase',penMod:'forge',neuralFilter:'sensor',capacitorPack:'energy',
  plasmaRifle:'energy',gravLance:'grav',swarmRifle:'bio',nanoSuit:'bio',gravRig:'grav',phaseShield:'phase',starShell:'star',quantumVisor:'phase',
  nanoWeaveGloves:'bio',phaseGrip:'phase',neuralMesh:'bio',echoMemory:'phase',timeLagModule:'phase',vacuumCarbine:'grav',exoShell:'star'
};
...
function techNodeEl(tid,p,focus){
  ...
  n.onclick=()=>{ if(state._moved) return; state.techSel = state.techSel===tid?null:tid; state.techJump=0; refreshTechSelection(); };
  return n;
}
...
function renderTechDetail(box){
  ...
  const x=el('button','tdet-x ui-icon-button',uiIcon('close')); x.setAttribute('aria-label','关闭科技详情'); x.onclick=()=>{ state.techSel=null; refreshTechSelection(); }; head.appendChild(x);
  d.appendChild(head);
...
function refreshTechSelection(){
  const cv=document.querySelector('.treecanvas'),detail=document.querySelector('.tech-det');if(!cv||!detail||!detail.replaceWith){render();return;}const focus=techLitSet();cv.classList.toggle('has-focus',!!focus);cv.querySelectorAll('[data-tid]').forEach(node=>{node.classList.toggle('sel',node.dataset.tid===state.techSel);node.classList.toggle('out',!!focus&&!focus.has(node.dataset.tid));});const holder=el('div');renderTechDetail(holder);detail.replaceWith(holder.children[0]);drawTechLines();save();
}
function refreshTechPanel(){
  const box=$('panel'),vp=box&&box.querySelector&&box.querySelector('.treevp'),detail=box&&box.querySelector&&box.querySelector('.tech-det');if(!box||!vp||!detail||!vp.replaceWith||!detail.replaceWith){render();return;}const holder=el('div');renderTechPanel(holder),freshVp=holder.children[0],freshDetail=holder.children[1];vp.replaceWith(freshVp);detail.replaceWith(freshDetail);renderTop();renderTabbar();save();
}
...
function research(tid){ const t=TECHS[tid]; if(!t||techKnown(tid))return;
  ...
  divider(); log(msg,'good'); divider(); syncQuestProgress(true); refreshTechPanel(); }
...
function replaceMountedNode(current,fresh,fallbackParent){
  if(!fresh)return false;const parent=current&&current.parentNode||fallbackParent;if(!parent)return false;const mounted=current&&current.parentNode===parent?current:(parent.firstElementChild||(parent.children&&parent.children[0]));
  if(mounted&&parent.replaceChild)parent.replaceChild(fresh,mounted);else if(parent.appendChild)parent.appendChild(fresh);else return false;return true;
}
const STATION_UI={};
...
function buildRecipeWorkbenchDetail(entry,ui,opts,refresh,tone){
  const quantityLimit=()=>entry.maxQty==null?9999:Math.max(0,Math.floor(Number(entry.heldId?state.inv[entry.heldId]:entry.maxQty)||0)),detail=el('section','station-recipe-detail'),detailBody=el('div','station-detail-body');let entryMax=Math.max(1,quantityLimit());
  detailBody.appendChild(el('div','station-result','<span class="station-result-art ui-art-frame">'+itemUiIcon(entry.out)+'</span><span><small>SELECTED OUTPUT</small><b>'+entry.name+'</b><p>'+(entry.desc||ITEMS[entry.out].desc||'生产完成后自动放入背包')+'</p></span><em>持有 '+(state.inv[entry.out]||0)+'</em>'));
  const outputItem=ITEMS[entry.out],equipmentStats=outputItem&&outputItem.type==='equip'?statTags(outputItem).split(' ').filter(Boolean):[];
  if(equipmentStats.length)detailBody.appendChild(el('section','station-equipment-profile ui-card','<div><small>EQUIPMENT STATS</small><b>装备属性</b></div><div class="station-equipment-stats ui-stat-grid">'+equipmentStats.map(stat=>'<span class="ui-stat-chip">'+stat+'</span>').join('')+'</div>'));
  ...
  Object.entries(entry.cost||{}).forEach(([id,n])=>{const row=el('div','station-material ui-list-row'),art=el('span','station-material-art ui-art-frame',itemUiIcon(id)),copy=el('span','station-material-copy'),need=el('em','station-material-need');row.appendChild(art);row.appendChild(copy);row.appendChild(need);materials.appendChild(row);materialRefs[id]={row,copy,need,perBatch:n};});
  detailBody.appendChild(materials);
  const outputs=el('div','station-output-list'),outputRefs={};
  Object.entries(entry.outputs||{[entry.out]:1}).forEach(([id,n])=>{const row=el('span','station-output',itemUiIcon(id)+'<b>'+ITEMS[id].name+'</b>'),amount=el('em');row.appendChild(amount);outputs.appendChild(row);outputRefs[id]={row,amount,perBatch:n};});
  const batch=el('div','station-batch'),fixed=!!entry.fixed,input=el('input','station-quantity');
  batch.appendChild(el('span','station-batch-copy','<small>PRODUCTION BATCH</small><b>'+(fixed?'唯一组件':'生产批次')+'</b><em>'+(fixed?'该项目只能制造 1 件':'可直接输入，范围 1—'+entryMax)+'</em>'));
  const controls=el('div','station-quantity-controls');
  if(fixed)controls.appendChild(el('span','station-fixed-quantity','1'));
  else{
    [-10,-1].forEach(delta=>{const button=el('button','station-step',String(delta));button.type='button';button.onclick=()=>sync(ui.qty+delta);controls.appendChild(button);});
    input.type='number';input.min='1';input.max=String(entryMax);input.inputMode='numeric';input.setAttribute('aria-label','生产批次数量');controls.appendChild(input);
    [1,10].forEach(delta=>{const button=el('button','station-step','+'+delta);button.type='button';button.onclick=()=>sync(ui.qty+delta);controls.appendChild(button);});
  }
  ...
  const confirm=el('button','primary station-confirm',opts.confirm||'确认生产');confirm.type='button';confirm.onclick=()=>entry.run(fixed?1:ui.qty,refresh);detail.appendChild(detailBody);detail.appendChild(confirm);
  function sync(value){
    entryMax=Math.max(1,quantityLimit());const qty=fixed?1:Math.min(entryMax,batchQuantity(value));ui.qty=qty;if(!fixed){input.max=String(entryMax);input.value=qty;}
    const stock=detailBody.querySelector('.station-result>em'),description=detailBody.querySelector('.station-result p');if(stock)stock.textContent='持有 '+(state.inv[entry.out]||0);if(description&&entry.desc)description.textContent=entry.desc;
    Object.entries(materialRefs).forEach(([id,ref])=>{const have=state.inv[id]||0,need=ref.perBatch*qty,enough=have>=need;ref.row.className='station-material ui-list-row '+(enough?'enough':'short');ref.copy.innerHTML='<b>'+ITEMS[id].name+'</b><small>现有 '+have+' / 需 '+need+'</small>';ref.need.innerHTML='<small>本次</small><b>'+need+'</b>';});
    Object.entries(outputRefs).forEach(([id,ref])=>{ref.amount.textContent='×'+((entry.outputs&&entry.outputs[id]||ref.perBatch)*qty);});
    const ready=entry.ready!==false&&!entry.done&&quantityLimit()>0,affordable=canAfford(scaledCost(entry.cost,qty));confirm.disabled=!ready||!affordable;confirm.textContent=entry.done?'已完成':(opts.confirm||'确认生产')+' · '+qty+' 批';
  }
  detail._refreshStationDetail=()=>sync(ui.qty);sync(ui.qty);if(!fixed){input.oninput=()=>{if(input.value===''){confirm.disabled=true;return;}sync(input.value);};input.onblur=()=>sync(input.value);}
  if(!opts.detailParent)return detail;const detailShell=el('div','station-workbench station-detail-workbench '+tone);detailShell._refreshStationDetail=detail._refreshStationDetail;detailShell.appendChild(detail);return detailShell;
}
function renderRecipeWorkbench(parent,key,entries,opts){
  ...
  const ui=stationUiState(key,entries),scrollState=splitGroup||ui,products=el('div','station-product-grid'),productRefs=[],scrollHost=opts.scrollHost||(opts.detailParent&&parent.closest&&parent.closest('.recipe-station-top'))||(opts.detailParent&&document.querySelector('.recipe-station-top'));if(scrollHost&&!opts.scrollHost)opts.scrollHost=scrollHost;if(splitGroup&&!splitGroup.group)splitGroup.group=key;
  ...
  const refresh=()=>{if(scrollHost)scrollState.scroll=scrollHost.scrollTop;productRefs.forEach(({entry,tile})=>{if(entry.update)Object.assign(entry,entry.update());if(entry.fixed)entry.done=entry.done||ownsItem(entry.out);const held=state.inv[entry.heldId||entry.out]||0,stock=tile.querySelector('small');tile.classList.toggle('done',!!entry.done);if(stock)stock.textContent=entry.done?'已完成':entry.gate||((entry.heldId?'可处理 ':'持有 ')+held);});const container=opts.detailParent||shell,current=container.children[container.children.length-1];if(current&&current._refreshStationDetail)current._refreshStationDetail();
    if(key.startsWith('starDock:')){const dock=document.querySelector('.facility-section.shipyard'),assemble=dock&&dock.querySelector('.facility-main-action'),summary=dock&&dock.querySelector('.workbench-visual>div>b'),ready=shipReady();if(assemble){assemble.disabled=ready||SHIP_COMPONENTS.some(id=>!has(id));assemble.textContent=ready?'回声号已完成':'完成远征舰总装';}if(summary)summary.textContent=ready?'远征舰【回声号】已服役':'回声号总装 '+SHIP_COMPONENTS.filter(has).length+'/'+SHIP_COMPONENTS.length;}
    renderTop();save();};
  const clearSelection=()=>productRefs.forEach(ref=>{ref.tile.classList.remove('selected');ref.tile.setAttribute('aria-pressed','false');});
  const selectEntry=entry=>{if(entry.id===ui.id&&(!splitGroup||splitGroup.group===key))return;if(splitGroup){if(opts.detailParent._clearStationSelection)opts.detailParent._clearStationSelection();splitGroup.group=key;opts.detailParent._clearStationSelection=clearSelection;}ui.id=entry.id;ui.qty=1;productRefs.forEach(ref=>{const active=ref.entry.id===ui.id;ref.tile.classList.toggle('selected',active);ref.tile.setAttribute('aria-pressed',active?'true':'false');});const fresh=buildRecipeWorkbenchDetail(entry,ui,opts,refresh,tone),container=opts.detailParent||shell;replaceMountedNode(container.children[container.children.length-1],fresh,container);};
  entries.forEach(entry=>{const selected=entry.id===ui.id,held=state.inv[entry.heldId||entry.out]||0,heldLabel=entry.heldId?'可处理 ':'持有 ',tile=el('button','station-product ui-card'+(selected?' selected':'')+(entry.done?' done':'')+(entry.ready===false?' gated':''));tile.type='button';tile.setAttribute('aria-pressed',selected?'true':'false');tile.innerHTML='<span class="station-product-art ui-art-frame">'+itemUiIcon(entry.out)+'</span><b>'+entry.name+'</b><small>'+(entry.done?'已完成':entry.gate||(heldLabel+held))+'</small>';tile.onclick=()=>selectEntry(entry);productRefs.push({entry,tile});products.appendChild(tile);});
  picker.appendChild(products);parent.appendChild(shell);if(splitGroup&&splitGroup.group!==key){clearSelection();return;}if(splitGroup)opts.detailParent._clearStationSelection=clearSelection;
  const entry=entries.find(e=>e.id===ui.id)||entries[0],detailNode=buildRecipeWorkbenchDetail(entry,ui,opts,refresh,tone);(opts.detailParent||shell).appendChild(detailNode);
}
...
function renderDefenseWorkbench(parent,detailParent){
  ...
  const ui=stationUiState('watch:defenses',entries),products=el('div','station-product-grid'),scrollHost=(parent.closest&&parent.closest('.recipe-station-top'))||document.querySelector('.recipe-station-top');
  if(scrollHost){requestAnimationFrame(()=>{scrollHost.scrollTop=ui.scroll||0;});scrollHost.onscroll=()=>{ui.scroll=scrollHost.scrollTop;};}
  let refresh;const buildDetail=entry=>{const current=entry.built,preview=current?{key:entry.id,level:current.level+1}:{key:entry.id,level:1},cost=current?upCost(current):entry.type.build,can=canAfford(cost),detail=el('section','station-recipe-detail defense-detail'),body=el('div','station-detail-body');
    body.appendChild(el('div','station-result defense-result','<span class="station-result-art ui-art-frame">'+defenseUiIcon(entry.id)+'</span><span><small>'+(current?'ACTIVE DEFENSE':'CONSTRUCTION READY')+'</small><b>'+entry.type.name+'</b><p>'+(current?'当前 Lv '+current.level+' · '+defenseStatText(current):'建成后 '+defenseStatText(preview))+'</p></span><em>'+(current?'Lv '+current.level:'未建造')+'</em>'));
    const stats=[['combat','攻击',defAtk(preview)],['armor','防御',defArmor(preview)],['offhand','护盾',defShield(preview)],['locate','射程',defRange(preview)]].filter(row=>row[2]);body.appendChild(el('section','station-equipment-profile defense-stat-profile ui-card','<div><small>'+(current?'NEXT LEVEL':'INITIAL SPECS')+'</small><b>'+(current?'下一级属性':'建成属性')+'</b></div><div class="station-equipment-stats ui-stat-grid">'+stats.map(row=>'<span class="ui-stat-chip">'+uiIcon(row[0])+' '+row[1]+' '+row[2]+'</span>').join('')+'</div>'));
    body.appendChild(el('div','station-subhead','<span><small>MATERIAL INPUT</small><b>'+(current?'升级所需材料':'建造所需材料')+'</b></span><em>'+(can?'库存已满足':'库存不足')+'</em>'));const materials=el('div','station-material-grid');Object.entries(cost).forEach(([id,need])=>{const have=state.inv[id]||0;materials.appendChild(el('div','station-material ui-list-row '+(have>=need?'enough':'short'),'<span class="station-material-art ui-art-frame">'+itemUiIcon(id)+'</span><span class="station-material-copy"><b>'+ITEMS[id].name+'</b><small>现有 '+have+' / 需 '+need+'</small></span><em class="station-material-need"><small>本次</small><b>'+need+'</b></em>'));});body.appendChild(materials);
    const detailShell=el('div','station-workbench station-detail-workbench defense-workbench'),confirm=el('button','primary station-confirm',current?'升级至 Lv '+(current.level+1):'建造 '+entry.type.name);confirm.type='button';confirm.disabled=!can;confirm.onclick=()=>{const index=state.defenses.findIndex(defense=>defense.key===entry.id);if(index>=0)upgradeDefense(index,refresh);else buildDefense(entry.id,refresh);};detail.appendChild(body);detail.appendChild(confirm);detailShell.appendChild(detail);return detailShell;};
  const productRefs=[];entries.forEach(entry=>{const selected=entry.id===ui.id,tile=el('button','station-product defense-product ui-card'+(selected?' selected':'')+(entry.built?'':' gated'));tile.type='button';tile.setAttribute('aria-pressed',selected?'true':'false');tile.innerHTML='<span class="station-product-art defense-product-art ui-art-frame">'+defenseUiIcon(entry.id)+'</span><b>'+entry.type.name+'</b><small>'+(entry.built?'Lv '+entry.level:'待建造')+'</small>';tile.onclick=()=>{if(entry.id===ui.id)return;ui.id=entry.id;productRefs.forEach(ref=>{const active=ref.entry.id===ui.id;ref.tile.classList.toggle('selected',active);ref.tile.setAttribute('aria-pressed',active?'true':'false');});replaceMountedNode(detailParent.children[0],buildDetail(entry),detailParent);};productRefs.push({entry,tile});products.appendChild(tile);});
  picker.appendChild(products);parent.appendChild(shell);refresh=()=>{
    if(scrollHost)ui.scroll=scrollHost.scrollTop;productRefs.forEach(({entry,tile})=>{entry.built=state.defenses.find(defense=>defense.key===entry.id);entry.level=entry.built?entry.built.level:0;tile.classList.toggle('gated',!entry.built);const level=tile.querySelector('small');if(level)level.textContent=entry.built?'Lv '+entry.level:'待建造';});
    const current=detailParent.children[0],fresh=buildDetail(entries.find(item=>item.id===ui.id)||entries[0]);
    ['.defense-result small','.defense-result p','.defense-result>em','.defense-stat-profile>div>small','.defense-stat-profile>div>b','.station-subhead b','.station-subhead>em'].forEach(selector=>{const old=current.querySelector(selector),next=fresh.querySelector(selector);if(old&&next)old.textContent=next.textContent;});
    const oldStats=current.querySelector('.station-equipment-stats'),newStats=fresh.querySelector('.station-equipment-stats');if(oldStats&&newStats)oldStats.innerHTML=newStats.innerHTML;
    const oldMaterials=current.querySelector('.station-material-grid'),newMaterials=fresh.querySelector('.station-material-grid');if(oldMaterials&&newMaterials){const keys=node=>Array.from(node.querySelectorAll('.station-material-copy b')).map(item=>item.textContent).join('|');if(keys(oldMaterials)!==keys(newMaterials))oldMaterials.replaceChildren(...newMaterials.children);else Array.from(oldMaterials.children).forEach((row,index)=>{const next=newMaterials.children[index];row.className=next.className;['.station-material-copy small','.station-material-need b'].forEach(selector=>{const value=row.querySelector(selector),updated=next.querySelector(selector);if(value&&updated)value.textContent=updated.textContent;});});}
    const confirm=current.querySelector('.station-confirm'),nextConfirm=fresh.querySelector('.station-confirm');if(confirm&&nextConfirm){confirm.disabled=nextConfirm.disabled;confirm.textContent=nextConfirm.textContent;}
    const summary=parent.querySelector('.watch-console>div>b'),header=document.querySelector('.facility-header-summary'),b=CAMP_BUILDINGS.find(building=>building.kind==='defense');if(summary){const guard=campDefenseStats();summary.textContent='攻击 '+guard.attack+' · 防御 '+guard.defense+' · 护盾 '+guard.shield;}if(header&&b)header.innerHTML=facilityHeaderDetail(b);renderTop();save();
  };detailParent.appendChild(buildDetail(entries.find(item=>item.id===ui.id)||entries[0]));
}
...
function renderGardenWorkbench(parent,detailParent){
  const plots=normalizeGardenPlots(),lv=buildingLevel('garden'),crops=GARDEN_CROPS.filter(c=>c.level<=lv),ui=stationUiState('garden:crops',crops),shell=el('div','garden-workbench');
  shell.appendChild(el('div','station-heading garden-heading','<span><small>BIOCULTURE DECK</small><b>培养与播种</b></span><em>作物按游戏时间生长</em>'));
  const beds=el('div','garden-bed-grid'),bedRefs=[];
  const replaceBed=index=>{const fresh=makeBed(index);replaceMountedNode(bedRefs[index],fresh,beds);bedRefs[index]=fresh;};const refreshBed=index=>{replaceBed(index);const selected=gardenCrop(ui.id)||crops[0],materials=detailParent.querySelectorAll('.station-material');Object.entries(selected.cost).forEach(([id,n],i)=>{const row=materials[i],copy=row&&row.querySelector('.station-material-copy small');if(row)row.className='station-material '+((state.inv[id]||0)>=n?'enough':'short');if(copy)copy.textContent='现有 '+(state.inv[id]||0)+' / 需 '+n;});renderTop();save();};
  const makeBed=index=>{const plot=normalizeGardenPlots()[index],crop=plot&&gardenCrop(plot.crop),selected=gardenCrop(ui.id)||crops[0],ready=!!plot&&state.time>=plot.readyAt,remain=plot?Math.max(0,plot.readyAt-state.time):0,status=sporeBoostStatus(index),clickable=ready||!plot,bed=el('article','garden-bed ui-card '+(ready?'ready':plot?'growing':'empty')),main=el(clickable?'button':'div','garden-bed-main',plot?gardenCropUiIcon(crop)+'<b>'+crop.name+'</b><small>'+(ready?'收获培养物':'剩余 '+remain+' 小时')+'</small>':uiIcon('plus')+'<b>空培养槽</b><small>播种 '+selected.name+'</small>'),skill=el('button','garden-bed-skill'+(status.ok?' active':''),uiIcon('skill-spore-boost')+'<span><b>催生孢子</b><small>'+(!plot?'播种后可用':ready?'培养物已成熟':status.ok?'体力 '+status.cost+' · 推进 '+sporeBoostHours()+' 小时':status.text)+'</small></span>');
    bed.dataset.slot=String(index);if(clickable){main.type='button';main.onclick=()=>ready?collectGardenPlot(index,false,()=>refreshBed(index)):plantGarden(index,selected.id,()=>refreshBed(index));}skill.type='button';skill.disabled=!status.ok;if(status.ok)skill.onclick=()=>performSporeBoost(index,()=>refreshBed(index));bed.append(main,skill);return bed;};
...
function renderDroneWorkbench(parent,detailParent){
  ...
  let refresh;const buildDetail=model=>{const detail=el('section','station-recipe-detail drone-detail'),body=el('div','station-detail-body'),perCycle=droneYield(model),fleetCycle=droneYield(model,fleet[model.id]);body.appendChild(el('div','station-result','<span class="station-result-art ui-art-frame">'+uiIcon(model.icon)+'</span><span><small>SELECTED DRONE</small><b>'+model.name+'</b><p>'+model.desc+'</p></span><em>持有 '+fleet[model.id]+'</em>'));
    body.appendChild(el('section','station-equipment-profile ui-card','<div><small>AUTO COLLECTION</small><b>自动采集属性</b></div><div class="station-equipment-stats ui-stat-grid"><span class="ui-stat-chip">周期 '+DRONE_CYCLE_HOURS+' 小时</span><span class="ui-stat-chip">单架 '+Object.entries(perCycle).map(([id,n])=>ITEMS[id].name+'×'+n).join(' · ')+'</span><span class="ui-stat-chip">当前机群 '+(fleet[model.id]?Object.entries(fleetCycle).map(([id,n])=>ITEMS[id].name+'×'+n).join(' · '):'暂无产出')+'</span></div>'));
    body.appendChild(el('div','station-subhead','<span><small>MATERIAL INPUT</small><b>建造所需材料</b></span><em>数量可批量设置</em>'));const materials=el('div','station-material-grid'),refs={};Object.entries(model.cost).forEach(([id,n])=>{const row=el('div','station-material ui-list-row'),copy=el('span','station-material-copy'),need=el('em','station-material-need');row.appendChild(el('span','station-material-art ui-art-frame',itemUiIcon(id)));row.appendChild(copy);row.appendChild(need);materials.appendChild(row);refs[id]={row,copy,need,n};});body.appendChild(materials);
    const batch=el('div','station-batch'),controls=el('div','station-quantity-controls'),input=el('input','station-quantity');batch.appendChild(el('span','station-batch-copy','<small>BUILD BATCH</small><b>建造数量</b><em>数量越多，自动采集产量越高</em>'));[-10,-1].forEach(delta=>{const button=el('button','station-step',String(delta));button.type='button';button.onclick=()=>sync(ui.qty+delta);controls.appendChild(button);});input.type='number';input.min='1';input.max='9999';input.inputMode='numeric';controls.appendChild(input);[1,10].forEach(delta=>{const button=el('button','station-step','+'+delta);button.type='button';button.onclick=()=>sync(ui.qty+delta);controls.appendChild(button);});batch.appendChild(controls);body.appendChild(batch);
    const confirm=el('button','primary station-confirm','建造无人机');confirm.type='button';confirm.onclick=()=>buildDrone(model.id,ui.qty,refresh);detail.appendChild(body);detail.appendChild(confirm);const detailShell=el('div','station-workbench station-detail-workbench drone-workbench');detailShell.appendChild(detail);function sync(value){ui.qty=batchQuantity(value);input.value=ui.qty;Object.entries(refs).forEach(([id,ref])=>{const have=state.inv[id]||0,need=ref.n*ui.qty,enough=have>=need;ref.row.className='station-material ui-list-row '+(enough?'enough':'short');ref.copy.innerHTML='<b>'+ITEMS[id].name+'</b><small>现有 '+have+' / 需 '+need+'</small>';ref.need.innerHTML='<small>本次</small><b>'+need+'</b>';});const held=body.querySelector('.station-result>em'),yieldText=body.querySelector('.ui-stat-chip:last-child');if(held)held.textContent='持有 '+fleet[model.id];if(yieldText)yieldText.textContent='当前机群 '+(fleet[model.id]?Object.entries(droneYield(model,fleet[model.id])).map(([id,n])=>ITEMS[id].name+'×'+n).join(' · '):'暂无产出');confirm.disabled=!canAfford(scaledCost(model.cost,ui.qty));confirm.textContent='建造 '+model.name+' · '+ui.qty+' 架';}detailShell._refreshStationDetail=()=>sync(ui.qty);sync(ui.qty);input.oninput=()=>{if(input.value===''){confirm.disabled=true;return;}sync(input.value);};input.onblur=()=>sync(input.value);return detailShell;};
  const products=el('div','station-product-grid drone-product-grid'),productRefs=[];entries.forEach(model=>{const active=model.id===selected.id,tile=el('button','station-product drone-product ui-card'+(active?' selected':''));tile.type='button';tile.setAttribute('aria-pressed',active?'true':'false');tile.innerHTML='<span class="station-product-art ui-art-frame">'+uiIcon(model.icon)+'</span><b>'+model.name+'</b><small>已建造 '+fleet[model.id]+'</small>';tile.onclick=()=>{if(model.id===ui.id)return;ui.id=model.id;ui.qty=1;productRefs.forEach(ref=>{const selected=ref.model.id===ui.id;ref.tile.classList.toggle('selected',selected);ref.tile.setAttribute('aria-pressed',selected?'true':'false');});replaceMountedNode(detailParent.children[0],buildDetail(model),detailParent);};productRefs.push({model,tile});products.appendChild(tile);});picker.appendChild(products);shell.appendChild(picker);parent.appendChild(shell);
  refresh=()=>{productRefs.forEach(({model,tile})=>{const count=tile.querySelector('small');if(count)count.textContent='已建造 '+fleet[model.id];});const total=picker.querySelector('.station-heading>em');if(total)total.textContent=Object.values(fleet).reduce((sum,n)=>sum+n,0)+' 架在线 · 每 '+DRONE_CYCLE_HOURS+' 小时自动入库';const detail=detailParent.children[0];if(detail&&detail._refreshStationDetail)detail._refreshStationDetail();renderTop();save();};detailParent.appendChild(buildDetail(selected));
}
function renderBuilding(box,id){
  ...
  if(b.kind==='rest'){
    ...
    const entries=SMELT.filter(s=>hasSmeltTech(s.id)).map(s=>{const levelOk=buildingLevel('smelt')>=(s.level||1),out=smeltOutput(s);return {id:s.id,out:s.out,name:s.name,desc:'每批产出 '+ITEMS[s.out].name+'×'+out,cost:s.cost,outputs:{[s.out]:out},ready:levelOk,gate:levelOk?'':'需要熔炼炉 Lv'+(s.level||1),update:()=>{const amount=smeltOutput(s);return {desc:'每批产出 '+ITEMS[s.out].name+'×'+amount,outputs:{[s.out]:amount}};},run:(qty,refresh)=>smelt(s,qty,refresh)};});
    ...
    sec.innerHTML='<div class="workbench-visual"><span>'+uiIcon('construct')+'</span><div><small>ASSEMBLY QUEUE</small><b>'+({work:'工程装配台',armor:'防护裁剪台',chem:'无菌调配台',elec:'晶圆与超导台',data:'量子演算台',energy:'聚变封装台',printer:'分子打印阵列',bio:'生物构造阵列',echo:'回响观测阵列',field:'重力场装配环'}[b.st]||'制造终端')+'</b><em>选择已解锁配方，材料会在制作时扣除。</em></div></div>';
    const entries=Object.keys(RECIPES).filter(rid=>RECIPES[rid].st===b.st&&hasRecipeTech(rid)).map(rid=>{const r=RECIPES[rid],it=ITEMS[r.out],levelOk=recipeFacilityReady(r);return {id:rid,out:r.out,name:it.name,desc:it.desc||'制造完成后自动放入背包',cost:r.cost,outputs:{[r.out]:r.yield||1},ready:levelOk,gate:levelOk?'':recipeFacilityText(r),run:(qty,refresh)=>craft(r,qty,refresh)};}),presentation=craftStationPresentation(b);
    ...
    const ready=shipReady(),owned=SHIP_COMPONENTS.filter(has).length;sec.innerHTML='<div class="workbench-visual"><span>'+uiIcon('expedition')+'</span><div><small>STARSHIP ASSEMBLY</small><b>'+(ready?'远征舰【回声号】已服役':'回声号总装 '+owned+'/'+SHIP_COMPONENTS.length)+'</b><em>五个系统来自不同科技分支；所有组件齐备后才能进行首航。</em></div></div>';
    const components=SHIP_COMPONENTS.filter(hasRecipeTech).map(id=>{const r=RECIPES[id],levelOk=recipeFacilityReady(r);return {id,out:id,name:ITEMS[id].name,desc:ITEMS[id].desc,cost:r.cost,outputs:{[id]:1},ready:levelOk,gate:levelOk?'':recipeFacilityText(r),done:has(id)||ready,fixed:true,run:(_,refresh)=>craft(r,1,refresh)};});
    ...
    const assemble=el('button','primary facility-main-action',ready?'回声号已完成':'完成远征舰总装');assemble.disabled=ready||SHIP_COMPONENTS.some(id=>!has(id));assemble.onclick=()=>assembleStarship(()=>refreshFacilityOperation(b.id));sec.appendChild(assemble);
    const payloads=Object.keys(RECIPES).filter(id=>RECIPES[id].st==='ship'&&!SHIP_COMPONENTS.includes(id)&&hasRecipeTech(id));if(payloads.length){const payloadEntries=payloads.map(id=>{const r=RECIPES[id],levelOk=recipeFacilityReady(r);return {id,out:id,name:ITEMS[id].name,desc:ITEMS[id].desc,cost:r.cost,outputs:{[id]:1},ready:levelOk,gate:levelOk?'':recipeFacilityText(r),done:has(id),fixed:true,run:(_,refresh)=>craft(r,1,refresh)};});renderRecipeWorkbench(sec,b.id+':payloads',payloadEntries,{tone:'ship',code:'ORBITAL PAYLOAD',title:'选择轨道装备',confirm:'确认装配',detailParent,splitKey});}
  } else if(b.kind==='nav'){
    ...
    sec.innerHTML='<div class="recycle-visual"><span>'+uiIcon('refresh')+'</span><div><small>MATERIAL & EQUIPMENT RECOVERY</small><b>拆解线待命</b><em>材料和背包中的闲置装备都能批量拆解；已穿戴装备不会出现在列表中。</em></div></div>';const bonus=buildingLevel('recycler')-1,mult=1+effectiveEconomyBonus('recyclePct')/100,recycleEntries=RECYCLE.filter(r=>r.level<=buildingLevel('recycler')).map(r=>{const outputs=Object.fromEntries(Object.entries(r.out).map(([id,n])=>[id,Math.max(1,Math.round((n+(id==='scrap'?bonus:0))*mult))])),out=Object.keys(outputs)[0],heldId=Object.keys(r.cost)[0];return {id:r.id,out,heldId,name:r.name,desc:'每批回收 '+Object.entries(outputs).map(([id,n])=>ITEMS[id].name+'×'+n).join(' · '),cost:r.cost,outputs,ready:true,run:(qty,refresh)=>recycleMaterial(r.id,qty,refresh)};});Object.keys(state.inv).filter(id=>state.inv[id]>0&&ITEMS[id]&&ITEMS[id].type==='equip').forEach(id=>{const outputs=equipmentRecycleYield(id);recycleEntries.push({id:'equip:'+id,out:id,heldId:id,name:'拆解：'+ITEMS[id].name,desc:'每件返还 '+Object.entries(outputs).map(([mat,n])=>ITEMS[mat].name+'×'+n).join(' · '),cost:{[id]:1},outputs,maxQty:state.inv[id],ready:true,run:(qty,refresh)=>recycleEquipment(id,qty,refresh)});});renderRecipeWorkbench(sec,b.id,recycleEntries,{tone:'recycle',code:'MATERIAL RECOVERY',title:'选择材料或闲置装备',confirm:'确认拆解',detailParent});
  } else if(b.kind==='mess'){
    ...
    const cookingEntries=Object.values(COOKING_RECIPES).map(r=>{const item=ITEMS[r.out],levelOk=lv>=r.level;return {id:r.out,out:r.out,name:item.name,desc:item.desc,cost:r.cost,outputs:{[r.out]:1},ready:levelOk,gate:levelOk?'':'需要营地厨房 Lv'+r.level,run:(qty,refresh)=>cookFood(r.out,false,qty,refresh)};});
...
/* ---------- 任务 ---------- */
const TASK_LINE_NAMES={main:'方舟主线',finale:'众证协议',space:'零号星门',survivor:'幸存者',evidence:'真相证据',surface:'地表行动',signal:'地下信号',special:'特殊蓝图'};
const TASK_LINE_CODES={main:'MAIN',finale:'WITNESS',space:'VOYAGE',survivor:'SURVIVOR',evidence:'EVIDENCE',surface:'SURFACE',signal:'SIGNAL',special:'SPECIAL'};
let taskBoardView='active',taskFocusId=null;
function taskQuestTarget(q){
  if(q.type==='npc')return finaleTaskStatus(q).at||null;
  if((q.type==='submit'||q.type==='choice')&&q.turnAt)return q.turnAt;
  return q.target&&LOCATIONS[q.target]?q.target:null;
}
function taskMaterialsReady(q){return !q.need||Object.entries(q.need).every(([id,n])=>(state.inv[id]||0)>=n);}
function taskReadyNow(q){
  if(!q||!questActive(q.id))return false;
  if(q.type==='submit')return P().location===q.turnAt&&taskMaterialsReady(q);
  if(q.type==='npc')return finaleTaskStatus(q).ok;
  const target=taskQuestTarget(q);return !!target&&target===P().location;
}
function taskPriorityScore(q){
  const target=taskQuestTarget(q),line=q.line==='main'?600:q.line==='finale'?500:q.line==='space'?420:0;
  return (taskReadyNow(q)?1000:0)+(target===P().location?240:0)+line-QUESTS.indexOf(q)/1000;
}
function taskPriorityQuest(quests){
  const focused=quests.find(q=>q.id===taskFocusId);if(focused)return focused;
  return [...quests].sort((a,b)=>taskPriorityScore(b)-taskPriorityScore(a))[0]||null;
}
function taskNeedProgress(q){return Object.entries(q.need||{}).map(([id,n])=>ITEMS[id].name+' '+Math.min(state.inv[id]||0,n)+'/'+n).join(' · ');}
function taskProgressText(q){
  if(q.type==='submit')return taskNeedProgress(q);
  const progress=questProgress(q);return progress||'等待任务条件达成';
}
function taskRewardText(q){
  const rewards=[];
  if(q.reward&&q.reward.items)Object.entries(q.reward.items).forEach(([id,n])=>rewards.push(ITEMS[id].name+' ×'+n));
  if(q.reward&&q.reward.reveal&&LOCATIONS[q.reward.reveal])rewards.push('解锁 '+LOCATIONS[q.reward.reveal].name);
  if(q.calibrates&&ITEMS[q.calibrates])rewards.push('校准 '+ITEMS[q.calibrates].name);
  return rewards.length?rewards.join(' · '):'推进'+(TASK_LINE_NAMES[q.line]||'任务线');
}
function taskNextStep(q){
  const target=taskQuestTarget(q),place=target&&LOCATIONS[target]?LOCATIONS[target].name:'';
  if(q.type==='submit'){
    if(!taskMaterialsReady(q))return '先补齐任务材料：'+taskNeedProgress(q);
    return P().location===q.turnAt?'材料齐备，现在可以交付。':'材料齐备，前往【'+place+'】交付。';
  }
  if(q.type==='npc')return finaleTaskStatus(q).text;
...
function renderTaskArchive(box,done){
  ...
  const records=el('div','task-record-grid');
  const fragments=el('section','task-record-card ui-card');fragments.innerHTML='<small>TRUTH FRAGMENTS</small><b>真相碎片 '+fragmentCount()+'/3</b><p>'+(['故障线','内鬼线','信号线'].map(line=>(state.meta.fragments.includes(line)?'✓ ':'○ ')+line).join(' · '))+'</p>';records.appendChild(fragments);
  const endingCount=state.meta.endingsDone.length,endings=Object.entries(ENDINGS).filter(([id,e])=>(e.selectable!==false||state.meta.endingsDone.includes(id))&&(id!=='cycle'||fragmentCount()>=3||state.meta.endingsDone.includes(id))).length;
  const ending=el('section','task-record-card ui-card');ending.innerHTML='<small>ENDING ARCHIVE</small><b>结局收集 '+endingCount+'/'+endings+'</b><p>'+(endingCount?state.meta.endingsDone.map(id=>'✓ '+ENDINGS[id].name).join(' · '):'尚未记录结局')+'</p>';records.appendChild(ending);box.appendChild(records);
...
function openLocalExport(){
  ...
  card.classList.add('backup-password-card');fields.append(first.label,second.label);create.onclick=async()=>{try{if(first.input.value!==second.input.value)throw new Error('两次输入的备份密码不一致');create.disabled=true;create.textContent='正在加密…';const text=await createLocalBackup(first.input.value);first.input.value='';second.input.value='';showLocalExportResult(text);}catch(error){result.textContent=error.message;result.className='save-transfer-result danger';create.disabled=false;create.textContent='加密并生成备份';}};cancel.onclick=closeSaveTransfer;actions.append(create,cancel);card.append(fields,result,actions);first.input.focus();
}
...
function restoreLocalRollback(){
  const raw=localStorage.getItem(LOCAL_ROLLBACK_KEY);if(!raw)return;let rollback;try{rollback=JSON.parse(raw);if(!validGameSave(rollback))throw new Error();}catch(_){const card=saveTransferShell('无法恢复回滚存档','本机回滚副本已损坏或格式不正确。');card.appendChild(el('p','save-transfer-result danger','当前进度没有被修改。'));return;}
...
function battleActionButton(action){
  const b=el('button','battle-action'+(action.cls?' '+action.cls:''));
  b.type='button';
  b.innerHTML=(action.code?'<span class="battle-action-code" aria-hidden="true">'+action.code+'</span>':'')+
    '<span class="battle-action-glyph" aria-hidden="true">'+(action.icon||uiIcon('combat'))+'</span><span class="battle-action-copy"><b>'+action.label+'</b><small>'+action.meta+'</small></span>';
...
function renderCombatPanel(box){
  ...
  const weaponId=P().equip.weapon,weaponVisual=weaponId&&ITEMS[weaponId]?itemUiIcon(weaponId):uiIcon('combat');
  const screen=el('div','battle-screen'+(c.boss?' is-boss':'')+(c.mech?' is-mech':' is-bio')+(c.id==='beacon'?' is-phantom':'')+((c.playerTurns||0)===0?' is-opening':'')),art=el('div','battle-art'),portrait=document.createElement('img'),hud=el('section','battle-hud');
  screen.dataset.enemy=enemyPortraitId(c.id);art.setAttribute('aria-hidden','true');portrait.src=enemyPortraitSrc(c.id);portrait.alt='';portrait.draggable=false;portrait.loading='eager';portrait.decoding='async';art.appendChild(portrait);screen.appendChild(art);appendCombatEffects(screen);

  const enemyStates=[c.boss?'首领':null,c.mech?'机械单位':'生物单位',c.infect?'感染风险':null,c.armorSegments>0?'场锚 '+c.armorSegments:null,c.barrier>0?'屏障 '+c.barrier:null,c.barrierEvery?'屏障重构':null,c.bombardEvery?'投射蓄能 '+((c.round||0)%c.bombardEvery+1)+'/'+c.bombardEvery:null,c.regenPct?'活体再生':null,c.enraged?'过载':c.enragePct?'阶段过载':null,c.staminaDrainEvery?'相位抽取':null,c.empTurns>0?'瘫痪 '+c.empTurns:null].filter(Boolean);
  const target=el('header','battle-target-hud');
  target.innerHTML='<div class="battle-target-lock"><span><i></i>TARGET ACQUIRED</span><em>'+(c.boss?'PRIORITY // ALPHA':'HOSTILE // CONTACT')+'</em></div>'+
    '<div class="battle-target-title"><span><small>敌对实体</small><b>'+c.name+'</b></span><strong><small>THREAT</small><b>'+String(c.threat||0).padStart(2,'0')+'</b></strong></div>'+
    '<div class="battle-target-health"><span><b>'+enemyHp+'</b><small>/ '+c.maxHp+' HP</small></span><em><i style="width:'+enemyPct+'%"></i></em><strong>'+Math.round(enemyPct)+'%</strong></div>'+
    '<div class="battle-target-data"><span class="battle-target-stats"><i>ATK '+c.atk+'</i><i>DEF '+c.def+'</i><i>RNG '+c.range+'</i></span><span class="battle-target-tags">'+enemyStates.map(x=>'<i>'+x+'</i>').join('')+'</span></div>';
  hud.appendChild(target);

  const rangeHud=el('aside','battle-range-core '+(canHit?'is-locked':'is-distant'));
  rangeHud.innerHTML='<div class="battle-range-ring"><span><small>DIST</small><b>'+c.distNow+'</b></span><i></i><span><small>RANGE</small><b>'+atkRange()+'</b></span></div><strong>'+(canHit?'LOCKED // 可攻击':'CLOSE IN // 需要接近')+'</strong>';
  hud.appendChild(rangeHud);

  const feed=el('div','battle-event-feed'),history=(c.history||[]).slice(-2);
  ...
  utility.appendChild(battleActionButton({label:'接近',meta:'距离 -'+moveRange()+' · '+(approachHpCost?'生命 -'+approachHpCost:'体力 -2'),icon:uiIcon('locate'),disabled:c.distNow<=1,fn:approach,cls:'battle-tool'}));
  const inSpace=['orbit','ashMoon','verdant','silent'].includes(regionForLocation(P().location));if(c.boss&&inSpace&&has('orbitalLance')&&shipReady())utility.appendChild(battleActionButton({label:'轨道',meta:c.orbitalUsed?'本场已调用':'破除场锚并削甲',icon:uiIcon('lance'),disabled:c.orbitalUsed,fn:orbitalStrike,cls:'battle-tool is-accent'}));
  if(c.mech)utility.appendChild(battleActionButton({label:'电磁',meta:'库存 '+(state.inv.emp||0)+' · 瘫痪3回合',icon:uiIcon('sensor'),disabled:!has('emp'),fn:()=>combatItem('emp'),cls:'battle-tool'}));
  utility.appendChild(battleActionButton({label:'急救',meta:'急救包 '+(state.inv.medkit||0),icon:uiIcon('medical'),disabled:!has('medkit'),fn:()=>combatItem('medkit'),cls:'battle-tool'}));
  utility.appendChild(battleActionButton({label:'补能',meta:'药剂 '+(state.inv.potion||0),icon:uiIcon('energy'),disabled:!has('potion'),fn:()=>combatItem('potion'),cls:'battle-tool'}));
  const fleeHpCost=movementHealthCost(2);
  utility.appendChild(battleActionButton({label:c.truthFinal?'核心锁定':'撤离',meta:c.truthFinal?'最终协议已闭合 · 胜负都会触发结局':(fleeHpCost?'生命 -'+fleeHpCost:'体力 -2')+' · 速度影响成功率',icon:uiIcon(c.truthFinal?'lock':'chevron-left'),cls:'battle-tool is-danger',disabled:!!c.truthFinal,fn:flee}));
  dock.appendChild(el('div','battle-rail-label','TACTICAL SUPPORT // 战术支持'));dock.appendChild(utility);dock.appendChild(primary);screen.appendChild(dock);box.appendChild(screen);
}
...
function smelt(s,quantity,refresh){ const count=batchQuantity(quantity);if(!facilityOnline('smelt')){log('熔炼炉未建成或已受损。','warn');return false;}if(!hasSmeltTech(s.id)){log('需先研究【'+TECHS[TECH_FOR_SMELT[s.id]].n+'】。','warn');return false;}if(buildingLevel('smelt')<(s.level||1)){log('需要熔炼炉 Lv'+(s.level||1)+'。','warn');return false;}
  ...
  log('🔥 熔炼:'+Object.entries(totalCost).map(([k,v])=>ITEMS[k].name+'×'+v).join(' ')+' → '+ITEMS[s.out].name+'×'+out,'good'); advanceTime(count); if(refresh)refresh();else render();return true; }
...
function performLocationAction(id){
  ...
  if(!locationActionRemaining(id)){log('这处场景行动今天已经达到上限，次日会刷新。','dim');refreshFieldAfterAction(id);return false;}
  const status=locationActionStatus(id);if(!status.ok){log(status.text+'。','warn');refreshFieldAfterAction(id);return false;}
  ...
  checkStamina();refreshFieldAfterAction(id);return true;
}
...
function performFieldGatherSkill(skillId){
  const id=P().location,status=fieldGatherSkillStatus(skillId,id),skill=SKILLS[skillId];if(!status.ok){log(status.text+'。','warn');refreshFieldAfterAction(id);return false;}
  ...
  syncQuestProgress(true);checkStamina();refreshFieldAfterAction(id);return true;
}
...
function explore(mode){
  ...
  if(mode==='gather'&&!resourceSiteDiscovered(id)){log('先继续探索，发现并登记资源点后才能定向采集。','warn');refreshFieldAfterAction(id);return;}
  if(mode==='gather'&&!gatherAvailable(id)){log('资源点储量耗尽，约 '+resourceRecoveryRemaining(id)+' 小时后恢复一次采集量。','dim');refreshFieldAfterAction(id);return;}
  ...
  if(work&&!work.ok){log(work.text+'。','warn');refreshFieldAfterAction(id);return;}
  if(careerStatus&&!careerStatus.ok){log(careerStatus.text+'。','warn');refreshFieldAfterAction(id);return;}
  ...
  if(mode==='gather'){
    ...
    checkStamina(); refreshFieldAfterAction(id); return;
  }
  ...
  checkStamina(); refreshFieldAfterAction(id);
}
...
function assembleStarship(refresh){
  ...
  divider();log('🚀 远征舰【回声号】完成总装。首航燃料与无条件紧急返航协议已装载。','good');divider();syncQuestProgress(true);updateCheckpoint();if(refresh)refresh();else render();
}
...
function buildDefense(key,refresh){ const t=DEF_TYPES[key]; if(!t)return; if(!hasDefTech(key)){log('需先研究对应科技。','warn');return;}
  ...
  log('建起了 '+t.icon+' '+t.name+'（'+defenseStatText({key,level:1})+'）。','good'); if(refresh)refresh();else render(); }
function upgradeDefense(i,refresh){ const d=state.defenses[i]; if(!d)return; const c=upCost(d);
  ...
  markInlineChange('defense',d.key);log('升级 '+DEF_TYPES[d.key].icon+DEF_TYPES[d.key].name+' → Lv'+d.level+'（'+defenseStatText(d)+'）。','good',{toast:false}); if(refresh)refresh();else render(); }
function rest(refresh){ finishExpedition();P().stamina=Math.round(maxStamina()); P().hp=P().infected?Math.max(P().hp,Math.round(maxHp()*.7)):maxHp(); P().shield=shieldMax(); state.rests++;
  ...
  updateCheckpoint(); log('你休息并记录了存档点。时间来到 '+fmtTime()+(P().infected?'；感染仍未清除。':'。'),'good'); if(refresh)refresh();else render(); }
...
function upgradeFacility(id){ const b=CAMP_BUILDINGS.find(x=>x.id===id),up=b&&facilityUpgrade(b); if(!b||!up)return;
  ...
  payCost(up.cost); state.meta.buildLevels[id]=buildingLevel(id)+1; advanceTime(3);persistMetaCheckpoint();state.siteSheet=null;markInlineChange('facility',id);log('⬆ 【'+b.name+'】升级为【'+up.name+'】。','good',{toast:false}); refreshFacilityOperation(id); }
function cookFood(id,skipRender,quantity,refresh){
  ...
  log('烹饪完成：【'+ITEMS[r.out].name+'】×'+count+' 已放入背包。','good');if(!skipRender){if(refresh)refresh();else render();}return true;
}
...
function sporeBoostHours(){const lv=Math.max(1,skillLv('sporeBoost'));return 2+Math.floor((lv-1)/5);}
function sporeBoostStatus(slot){
  const cost=SKILLS.sporeBoost.cost,plots=normalizeGardenPlots();if(!Number.isInteger(slot)||slot<0||slot>=plots.length)return {ok:false,text:'培养槽不存在'};const plot=plots[slot];
...
function enemyCombatProfile(e){
  return {hp:e.hp,atk:e.atk,def:e.def,bombardDamage:e.bombardDamage||0};
}
function startCombat(eid,extra){ const e=ENEMIES[eid];if(!e)return; combatFxQueue=[];state.tab='act'; P().shield=shieldMax();
...
function performAttack(mult,ignoreDef,label,forceCrit,options){ const c=state.combat; if(!c)return;options=options||{};
  ...
  if(statLS()>0&&dmg>0&&P().hp<maxHp()){
    ...
    const recovered=P().hp-before;if(recovered>0)log('嗜血回复 '+recovered+' 生命（'+P().hp+'/'+maxHp()+'）。','good');
  }
...
function boot(){
  const wire=(b,tabname)=>{ if(!b) return; b.onclick=()=>{ if(tutorialActive()||state.combat||state.screen!=='play')return; const active=primaryTabKey();state.campBuilding=null;state.siteSheet=null;if(active===tabname){state.tab='act';state.charView='overview';}else if(tabname==='career'||tabname==='skill'){state.tab='char';state.charView=tabname==='career'?'careers':'skills';if(tabname==='skill')state.skillView=state.skillView||'active';}else{state.tab=tabname;if(tabname==='char')state.charView='overview';}
