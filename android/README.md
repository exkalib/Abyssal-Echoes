...
这是一个原生 WebView 外壳。APK 内置 `prototype/` 当前版本，离线也能游玩。启动时先进入坠舰开屏并从 59 测试服务器校验签名更新清单：Wi-Fi 下自动更新，其他网络会先确认；没有更新或更新节点不可用时载入本地存档，再由玩家点击“进入游戏”。

游戏进度始终先保存在设备本地。Netlify 只提供手动迁移包：玩家明确点击上传、下载或查看上一版本时才调用接口；读取操作冷却 10 秒，上传、覆盖和恢复冷却 30 秒。正常游玩、切换前后台和退出 App 都不会连接云存档服务。
